...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Train the provisional sit-stand structured Logistic baseline."
    )
    parser.add_argument(
        "--data",
        type=Path,
...
def _load_training_config(path: Path, profile: str) -> SitStandLogisticConfig:
    ...
    if not isinstance(profiles, dict) or not isinstance(profiles.get(profile), dict):
        raise ValueError(f"sit-stand training config is missing profile {profile}")
    return SitStandLogisticConfig(**profiles[profile])


def main(argv: Sequence[str] | None = None) -> int:
    args = build_parser().parse_args(argv)
    try:
        summary = train_sit_stand_logistic(
            args.data,
            args.output_dir,
