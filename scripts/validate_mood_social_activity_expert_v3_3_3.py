"""Validate MODEL-001 artifacts, OOF coverage, and fold leakage boundaries."""
...
DEFAULT_CONFIG = Path("configs/training/mood_social_activity_expert_v3_3_3.yaml")
DEFAULT_REPORT = Path("reports/mental_health/mood_social/MH-20260731-001")
DEFAULT_MODEL = Path("models/mental_health/mood_social/v3.3.3/activity_expert.joblib")
...
def validate(
    ...
    inputs = load_activity_training_inputs(root)
    assignments = {
    ...
    _check(
        ...
        and model_manifest.get("dataset_ids") == list(ACTIVITY_DATASET_IDS),
    ...
    _check(isinstance(bundle, ActivityExpertBundle), "model_bundle_type", checks)
    ...
    _check(
        bundle.training_scope == "all_activity_sources_for_production"
    ...
    _check(
        ...
        == "mood-social-activity-oof-predictions-v1",
        "prediction_schema_version",
        checks,
    )
    _check(
        ...
        and set(predictions["dataset_id"].astype(str)) == set(ACTIVITY_DATASET_IDS),
        "prediction_coverage",
        checks,
    )
    _check(
    ...
    return {
        ...
        "dataset_count": len(ACTIVITY_DATASET_IDS),
...
def _validate_bundle_identity(
    ...
    _check(
        bundle.dataset_ids == ACTIVITY_DATASET_IDS
        and bundle.input_feature_names == ACTIVITY_FEATURE_COLUMNS
        and set(bundle.preprocessor.selected_features).issubset(
            ACTIVITY_FEATURE_COLUMNS
        )
        and not any(
...
def _validate_bundle_mask_behavior(
    ...
    values = {name: [np.nan] for name in ACTIVITY_FEATURE_COLUMNS}
    masks = {name: [0] for name in ACTIVITY_MASK_COLUMNS}
...
def _validate_outer_fold_bundles(
    ...
    for outer_fold in range(5):
        ...
        _check(
            isinstance(bundle, ActivityExpertBundle)
...
def _available_scope(
    ...
    for dataset_id in ACTIVITY_DATASET_IDS:
        ...
        masks = transformed[list(ACTIVITY_MASK_COLUMNS)].apply(
            pd.to_numeric,
            errors="raise",
        )
    ...
    if not available_keys or available_rows <= 0:
        raise ValidationError("ActivityExpert available training scope is empty")
...
def _validate_report_artifacts(report: Path, checks: list[str]) -> None:
    ...
    _check(
        ...
        == "mood-social-activity-report-artifacts-v1"
...
def _read_json(path: Path) -> Any:
    try:
        ...
        raise ValidationError("required MODEL-001 JSON is unreadable") from exc
...
def _sha256_file(path: Path, chunk_size: int = 1024 * 1024) -> str:
    ...
    try:
        ...
        raise ValidationError("required MODEL-001 file is unreadable") from exc
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"MODEL-001 validation failed: {exc}", file=sys.stderr)
        return 1
