    ...
    *,
    ...
    try:
        ...
        try:
            ...
            # Container fallback is useful only for decode failures.  A decoder that
            # already produced trustworthy metadata/samples may instead have raised a
            # contract validation error; do not replace that error with an incidental
            # missing-FFmpeg failure from the fallback path.
            if (
                "declared audio" in str(soundfile_error)
                or "duration must be between" in str(soundfile_error)
            ):
