from __future__ import annotations

import hashlib
import json
import os
from collections import Counter, defaultdict
from dataclasses import asdict, dataclass
from pathlib import Path
from typing import Any, Iterable, Mapping, Sequence

import numpy as np

from elderly_monitoring.modules.fall_risk.sit_stand import extract_sit_stand_events


SIT_STAND_JOINTS = (
    "left_shoulder",
    "right_shoulder",
    "left_elbow",
    "right_elbow",
    "left_wrist",
    "right_wrist",
    "left_hip",
    "right_hip",
    "left_knee",
    "right_knee",
    "left_ankle",
    "right_ankle",
    "pelvis_center",
    "shoulder_center",
)
SIT_STAND_CHANNELS = (
    "x_torso",
    "y_torso",
    "vx_torso_per_sec",
    "vy_torso_per_sec",
    "quality",
    "image_y",
    "valid_mask",
)
SIT_STAND_TABULAR_FEATURES = (
    "pelvis_image_y_range",
    "pelvis_image_y_net_change",
    "shoulder_image_y_range",
    "shoulder_image_y_net_change",
    "leg_extension_range",
    "trunk_forward_angle_p90",
    "pelvis_lateral_sway",
    "pelvis_vertical_speed_p90",
    "usable_frame_ratio",
    "core_joint_coverage",
    "mean_joint_quality",
    "interpolated_point_ratio",
    "jump_outlier_per_frame",
)

POSITIVE_ACTION_IDS = ("A03", "A04")
EXPLICIT_HARD_NEGATIVE_ACTION_IDS = (
    "A05",
    "A06",
    "A07",
    "A09",
    "A11",
    "D01",
    "D02",
    "D03",
    "D04",
    "D05",
)
SIT_STAND_AUDIT_ACTION_IDS = ("A03", "A04", "B06", "C01")
ALLOWED_ACTION_TIERS = {"primary", "auxiliary", "ignore"}
DEVELOPMENT_PARTITIONS = {"train", "validation"}
ALL_PARTITIONS = {"train", "validation", "test"}


@dataclass(frozen=True)
class SitStandDatasetConfig:
    target_fps: float = 4.0
    window_sec: float = 4.0
    stride_sec: float = 2.0
    max_gap_sec: float = 0.50
    min_observed_frames: int = 6
    min_usable_frame_ratio: float = 0.60
    min_core_joint_coverage: float = 0.70
    max_windows_per_event: int = 4
    seed: int = 42

    def __post_init__(self) -> None:
        if self.target_fps <= 0:
            raise ValueError("target_fps must be positive")
        if self.window_sec <= 0 or self.stride_sec <= 0:
            raise ValueError("window_sec and stride_sec must be positive")
        if self.max_gap_sec <= 0:
            raise ValueError("max_gap_sec must be positive")
        if self.window_frames < 2:
            raise ValueError("window contract must contain at least 2 frames")
        if not 1 <= self.min_observed_frames <= self.window_frames:
            raise ValueError("min_observed_frames must be within the window")
        if not 0 < self.min_usable_frame_ratio <= 1:
            raise ValueError("min_usable_frame_ratio must be within (0, 1]")
        if not 0 < self.min_core_joint_coverage <= 1:
            raise ValueError("min_core_joint_coverage must be within (0, 1]")
        if self.max_windows_per_event < 1:
            raise ValueError("max_windows_per_event must be positive")

    @property
    def window_frames(self) -> int:
        return int(round(self.target_fps * self.window_sec))

    @property
    def stride_frames(self) -> int:
        return max(1, int(round(self.target_fps * self.stride_sec)))


def build_sit_stand_tensor(
    records: Sequence[Mapping[str, Any] | None],
    *,
    window_frames: int,
    target_fps: float,
) -> np.ndarray:
    """Build the fixed sit-stand [T,14,7] pose contract."""

    if window_frames < 2:
        raise ValueError("window_frames must be at least 2")
    if target_fps <= 0:
        raise ValueError("target_fps must be positive")
    if len(records) > window_frames:
        raise ValueError("records exceed window_frames")

    joint_count = len(SIT_STAND_JOINTS)
    xy = np.zeros((window_frames, joint_count, 2), dtype=np.float32)
    quality = np.zeros((window_frames, joint_count), dtype=np.float32)
    valid = np.zeros((window_frames, joint_count), dtype=np.float32)
    for frame_index, record in enumerate(records):
        if record is None:
            continue
        points = {
            str(point.get("name")): point
            for point in record.get("keypoints", [])
            if isinstance(point, Mapping)
        }
        for joint_index, name in enumerate(SIT_STAND_JOINTS[:12]):
            point = points.get(name)
            if point is None or point.get("valid") is not True:
                continue
            if point.get("is_jump_outlier") is True:
                continue
            x = _optional_float(point.get("x_smooth", point.get("x")))
            y = _optional_float(point.get("y_smooth", point.get("y")))
            point_quality = _optional_float(
                point.get("quality_weight", point.get("score"))
            )
            if x is None or y is None or point_quality is None or point_quality <= 0:
                continue
            xy[frame_index, joint_index] = (x, y)
            quality[frame_index, joint_index] = min(1.0, max(0.0, point_quality))
            valid[frame_index, joint_index] = 1.0
        _derive_center_joint(xy, quality, valid, frame_index, 6, 7, 12)
        _derive_center_joint(xy, quality, valid, frame_index, 0, 1, 13)

    torso_lengths = np.linalg.norm(xy[:, 13] - xy[:, 12], axis=1)
    valid_scale = (
        (valid[:, 12] > 0)
        & (valid[:, 13] > 0)
        & np.isfinite(torso_lengths)
        & (torso_lengths > 1e-6)
    )
    tensor = np.zeros(
        (window_frames, joint_count, len(SIT_STAND_CHANNELS)), dtype=np.float32
    )
    if not valid_scale.any():
        return tensor

    body_scale = float(np.median(torso_lengths[valid_scale]))
    valid_center = valid[:, 12] > 0
    centered = np.zeros_like(xy)
    centered[valid_center] = (
        xy[valid_center] - xy[valid_center, 12:13]
    ) / body_scale
    centered[valid <= 0] = 0.0
    quality[~valid_center] = 0.0
    valid[~valid_center] = 0.0

    velocity = np.zeros_like(centered)
    if window_frames > 1:
        velocity[1:] = (centered[1:] - centered[:-1]) * float(target_fps)
        valid_pairs = (valid[1:] > 0) & (valid[:-1] > 0)
        velocity[1:][~valid_pairs] = 0.0

    image_y = xy[..., 1].copy()
    image_y[valid <= 0] = 0.0
    tensor[..., 0:2] = centered
    tensor[..., 2:4] = velocity
    tensor[..., 4] = quality
    tensor[..., 5] = image_y
    tensor[..., 6] = valid
    if not np.isfinite(tensor).all():
        raise ValueError("sit-stand tensor contains non-finite values")
    return tensor


def audit_sit_stand_training_inputs(
    *,
    labels: str | Path,
    manifest: str | Path,
    assignments: str | Path,
    split: str | Path,
    validation: str | Path,
    pose_dir: str | Path,
) -> dict[str, Any]:
    labels_path = Path(labels)
    manifest_path = Path(manifest)
    assignments_path = Path(assignments)
    split_path = Path(split)
    validation_path = Path(validation)
    pose_root = Path(pose_dir)
    for path in (
        labels_path,
        manifest_path,
        assignments_path,
        split_path,
        validation_path,
    ):
        if not path.is_file():
            raise FileNotFoundError(f"required sit-stand audit input not found: {path}")
    if not pose_root.is_dir():
        raise FileNotFoundError(f"sit-stand pose directory not found: {pose_root}")

    label_rows = _read_jsonl(labels_path)
    manifest_rows = _read_jsonl(manifest_path)
    assignment_rows = _read_jsonl(assignments_path)
    _validate_label_rows(label_rows)
    assignment_index = _assignment_index(assignment_rows)
    selected = [
        row for row in label_rows if str(row.get("action_id")) in SIT_STAND_AUDIT_ACTION_IDS
    ]
    _validate_assignment_links(selected, assignment_index)
    leakage_issues = _protection_unit_leakage(selected, assignment_index)
    if leakage_issues:
        first = leakage_issues[0]
        raise ValueError(
            "sit-stand partition leakage for "
            f"{first['field']}={first['value']}: {first['partitions']}"
        )

    split_report = json.loads(split_path.read_text(encoding="utf-8"))
    validation_report = json.loads(validation_path.read_text(encoding="utf-8"))
    _validate_input_hashes(
        labels_path=labels_path,
        manifest_path=manifest_path,
        assignments_path=assignments_path,
        split_path=split_path,
        split_report=split_report,
        validation_report=validation_report,
    )
    if split_report.get("leakage_issues"):
        raise ValueError("source split report contains leakage issues")

    manifest_index = _manifest_index(manifest_rows)
    action_summary: dict[str, Any] = {}
    for action_id in SIT_STAND_AUDIT_ACTION_IDS:
        rows = [row for row in selected if row.get("action_id") == action_id]
        action_summary[action_id] = _action_summary(
            rows, assignment_index, manifest_index, pose_root
        )

    explicit_background_count = sum(
        _is_explicit_background(row) for row in label_rows
    )
    linked_event_count = sum(
        bool(row.get("linked_event_id")) for row in selected
    )
    verified_event_boundary_count = sum(
        row.get("event_boundary_verified") is True
        or row.get("boundary_semantics") == "event_onset_offset"
        for row in selected
    )
    proxy_rows = [
        row
        for row in label_rows
        if str(row.get("action_id"))
        in {*POSITIVE_ACTION_IDS, *EXPLICIT_HARD_NEGATIVE_ACTION_IDS}
        and row.get("action_type_training_tier") == "primary"
    ]
    proxy_counts = _proxy_partition_counts(proxy_rows, assignment_index)
    missing_development_pose_ids = _missing_development_pose_ids(
        proxy_rows, assignment_index, pose_root
    )
    missing_development_pose = len(missing_development_pose_ids)
    proxy_ready = all(
        proxy_counts[partition][target] > 0
        for partition in ("train", "validation")
        for target in ("positive", "negative")
    ) and missing_development_pose == 0
    localization_ready = (
        explicit_background_count > 0
        and linked_event_count > 0
        and verified_event_boundary_count > 0
    )
    action_type_ready = bool(
        validation_report.get("training_ready", {}).get("action_type", False)
    )
    report = {
        "schema_version": "sit-stand-training-audit-v1",
        "task": "sit_stand_event_localization_v1",
        "status": "formal" if localization_ready and action_type_ready else "provisional",
        "input_sha256": {
            "labels": _sha256_file(labels_path),
            "manifest": _sha256_file(manifest_path),
            "assignments": _sha256_file(assignments_path),
            "split": _sha256_file(split_path),
            "validation": _sha256_file(validation_path),
        },
        "source_split_id": split_report.get("split_id"),
        "action_summary": action_summary,
        "proxy_partition_counts": proxy_counts,
        "evidence": {
            "explicit_background_label_count": explicit_background_count,
            "linked_event_label_count": linked_event_count,
            "verified_event_boundary_count": verified_event_boundary_count,
            "exact_action_clip_boundary_count": sum(
                row.get("boundary_precision") == "exact" for row in selected
            ),
            "single_annotated_count": sum(
                row.get("review_status") == "single_annotated" for row in selected
            ),
            "partition_leakage_issue_count": 0,
        },
        "training_gates": {
            "event_localization_ready": localization_ready,
            "event_presence_proxy_ready": proxy_ready,
            "action_type_formal_ready": action_type_ready,
            "phase_segmentation_ready": False,
            "functional_proxy_ready": False,
        },
        "data_access": {
            "development_pose_missing": missing_development_pose,
            "development_pose_missing_video_ids": missing_development_pose_ids,
            "test_pose_read": False,
        },
        "limitations": [
            "exact action clip boundaries are not treated as verified event onset/offset",
            "no unlabelled interval is inferred to be background",
            "test pose data is not opened during audit or provisional preparation",
            "the proxy can support clip-level pretraining only, not event localization metrics",
        ],
    }
    return report


def prepare_sit_stand_event_dataset(
    *,
    labels: str | Path,
    manifest: str | Path,
    assignments: str | Path,
    split: str | Path,
    validation: str | Path,
    pose_dir: str | Path,
    output_dir: str | Path,
    config: SitStandDatasetConfig | None = None,
    allow_provisional: bool = False,
) -> dict[str, Any]:
    preparation = config or SitStandDatasetConfig()
    if not allow_provisional:
        raise ValueError(
            "sit-stand event preparation is provisional; pass allow_provisional explicitly"
        )
    labels_path = Path(labels)
    manifest_path = Path(manifest)
    assignments_path = Path(assignments)
    split_path = Path(split)
    validation_path = Path(validation)
    pose_root = Path(pose_dir)
    destination = Path(output_dir)
    output_paths = {
        "audit": destination / "audit.json",
        "dataset": destination / "dataset.npz",
        "metadata": destination / "metadata.json",
        "samples": destination / "samples.jsonl",
        "split": destination / "split.json",
        "assignments": destination / "assignments.jsonl",
    }
    existing = [path for path in output_paths.values() if path.exists()]
    if existing:
        raise FileExistsError(f"sit-stand dataset output already exists: {existing[0]}")

    audit = audit_sit_stand_training_inputs(
        labels=labels_path,
        manifest=manifest_path,
        assignments=assignments_path,
        split=split_path,
        validation=validation_path,
        pose_dir=pose_root,
    )
    missing_pose_ids = audit["data_access"]["development_pose_missing_video_ids"]
    if missing_pose_ids:
        raise FileNotFoundError(
            "development pose-quality JSONL not found for "
            + ", ".join(str(value) for value in missing_pose_ids[:10])
        )
    if not audit["training_gates"]["event_presence_proxy_ready"]:
        raise ValueError("sit-stand event presence proxy data gate is not ready")
    label_rows = _read_jsonl(labels_path)
    manifest_index = _manifest_index(_read_jsonl(manifest_path))
    assignment_index = _assignment_index(_read_jsonl(assignments_path))
    selected_labels = [
        row
        for row in label_rows
        if str(row.get("action_id"))
        in {*POSITIVE_ACTION_IDS, *EXPLICIT_HARD_NEGATIVE_ACTION_IDS}
        and row.get("action_type_training_tier") == "primary"
    ]
    selected_labels.sort(
        key=lambda row: (
            str(row.get("video_id", "")),
            int(row.get("start_frame", 0)),
            str(row.get("label_id", "")),
        )
    )

    development_labels: list[dict[str, Any]] = []
    locked_test_labels: list[dict[str, Any]] = []
    for label in selected_labels:
        assignment = assignment_index[str(label["label_id"])]
        partition = str(assignment["partition"])
        row = dict(label)
        row["partition"] = partition
        row["split_group_id"] = str(assignment["split_group_id"])
        if partition in DEVELOPMENT_PARTITIONS:
            development_labels.append(row)
        elif partition == "test":
            locked_test_labels.append(row)

    pose_cache: dict[str, list[dict[str, Any]]] = {}
    pose_paths: dict[str, Path] = {}
    tensors: list[np.ndarray] = []
    tabular_features: list[list[float]] = []
    rule_scores: list[float] = []
    samples: list[dict[str, Any]] = []
    rejected: Counter[str] = Counter()
    for label in development_labels:
        video_id = str(label["video_id"])
        manifest_row = manifest_index.get(video_id)
        if manifest_row is None:
            raise ValueError(f"sit-stand label references missing manifest video: {video_id}")
        if manifest_row.get("eligibility") is not True:
            raise ValueError(f"sit-stand label references ineligible video: {video_id}")
        if str(manifest_row.get("asset_id", "")) != str(label.get("asset_id", "")):
            raise ValueError(f"sit-stand label/manifest asset mismatch: {video_id}")
        if video_id not in pose_cache:
            pose_path = _resolve_pose_path(pose_root, video_id)
            if pose_path is None:
                raise FileNotFoundError(
                    f"development pose-quality JSONL not found for {video_id}"
                )
            pose_paths[video_id] = pose_path
            pose_cache[video_id] = _read_jsonl(pose_path)
        track_records = _select_labeled_track(label, pose_cache[video_id])
        if not track_records:
            rejected["no_matching_pose_track"] += 1
            continue
        start_time = float(label["start_time"])
        end_time = float(label["end_time_exclusive"])
        segment_records = [
            record
            for record in track_records.values()
            if start_time <= float(record.get("timestamp_sec", -1.0)) < end_time
        ]
        if not segment_records:
            rejected["no_pose_in_label_time"] += 1
            continue
        window_starts = _window_start_times(start_time, end_time, preparation)
        for window_index, window_start in enumerate(window_starts):
            slots = _resample_pose_records(
                segment_records,
                start_time_sec=window_start,
                window_frames=preparation.window_frames,
                target_fps=preparation.target_fps,
                max_gap_sec=preparation.max_gap_sec,
            )
            observed = [record for record in slots if record is not None]
            if len(observed) < preparation.min_observed_frames:
                rejected["insufficient_observed_frames"] += 1
                continue
            quality = _window_quality(slots)
            if (
                quality["usable_frame_ratio"] < preparation.min_usable_frame_ratio
                or quality["core_joint_coverage"]
                < preparation.min_core_joint_coverage
            ):
                rejected["insufficient_quality"] += 1
                continue
            tensor = build_sit_stand_tensor(
                slots,
                window_frames=preparation.window_frames,
                target_fps=preparation.target_fps,
            )
            if not np.any(tensor[..., -1] > 0):
                rejected["empty_valid_mask"] += 1
                continue
            features = _tabular_features(tensor, slots, quality)
            rule = _rule_prediction(observed)
            action_id = str(label["action_id"])
            target = 1 if action_id in POSITIVE_ACTION_IDS else 0
            direction = (
                "stand_to_sit"
                if action_id == "A03"
                else "sit_to_stand"
                if action_id == "A04"
                else "non_sit_stand"
            )
            sample_id = f"{label['label_id']}:window-{window_index:03d}"
            sample = {
                "sample_id": sample_id,
                "event_id": str(label["label_id"]),
                "label_id": str(label["label_id"]),
                "video_id": video_id,
                "asset_id": str(label["asset_id"]),
                "action_id": action_id,
                "action_type": str(label["action_type"]),
                "direction": direction,
                "label": target,
                "target_name": "sit_stand_event" if target else "explicit_hard_negative",
                "partition": str(label["partition"]),
                "split_group_id": str(label["split_group_id"]),
                "source_group_id": str(label["source_group_id"]),
                "sample_group_id": str(label["sample_group_id"]),
                "subject_id": str(label.get("subject_id", "unknown")),
                "dataset": str(manifest_row.get("dataset", "unknown")),
                "scene_region": str(manifest_row.get("scene_region", "unknown")),
                "window_start_time_sec": round(window_start, 6),
                "window_end_time_exclusive": round(
                    window_start + preparation.window_sec, 6
                ),
                "observed_frame_count": len(observed),
                "quality": quality,
                "rule_event_detected": bool(rule["detected"]),
                "rule_direction": rule["direction"],
            }
            tensors.append(tensor)
            tabular_features.append(features)
            rule_scores.append(float(rule["detected"]))
            samples.append(sample)

    if not samples:
        raise ValueError("no usable sit-stand development windows were generated")
    _validate_development_samples(samples)
    event_window_counts = Counter(str(sample["event_id"]) for sample in samples)
    sample_weights = np.asarray(
        [1.0 / event_window_counts[str(sample["event_id"])] for sample in samples],
        dtype=np.float32,
    )
    _validate_event_weights(samples, sample_weights)

    destination.mkdir(parents=True, exist_ok=True)
    _write_json_atomic(output_paths["audit"], audit)
    _write_jsonl_atomic(output_paths["samples"], samples)
    derived_assignments = [
        {
            "schema_version": "sit-stand-provisional-split-assignment-v1",
            "label_id": str(row["label_id"]),
            "video_id": str(row["video_id"]),
            "split_group_id": str(assignment_index[str(row["label_id"])]["split_group_id"]),
            "source_group_id": str(row["source_group_id"]),
            "sample_group_id": str(row["sample_group_id"]),
            "subject_id": str(row.get("subject_id", "unknown")),
            "action_type_training_tier": str(row["action_type_training_tier"]),
            "partition": str(assignment_index[str(row["label_id"])]["partition"]),
            "features_materialized": (
                assignment_index[str(row["label_id"])]["partition"]
                in DEVELOPMENT_PARTITIONS
            ),
        }
        for row in selected_labels
    ]
    _write_jsonl_atomic(output_paths["assignments"], derived_assignments)

    arrays = {
        "features": np.stack(tensors).astype(np.float32),
        "tabular_features": np.asarray(tabular_features, dtype=np.float32),
        "labels": np.asarray([sample["label"] for sample in samples], dtype=np.int64),
        "partitions": np.asarray([sample["partition"] for sample in samples]),
        "sample_ids": np.asarray([sample["sample_id"] for sample in samples]),
        "event_ids": np.asarray([sample["event_id"] for sample in samples]),
        "video_ids": np.asarray([sample["video_id"] for sample in samples]),
        "split_group_ids": np.asarray([sample["split_group_id"] for sample in samples]),
        "source_group_ids": np.asarray([sample["source_group_id"] for sample in samples]),
        "sample_group_ids": np.asarray([sample["sample_group_id"] for sample in samples]),
        "datasets": np.asarray([sample["dataset"] for sample in samples]),
        "action_ids": np.asarray([sample["action_id"] for sample in samples]),
        "directions": np.asarray([sample["direction"] for sample in samples]),
        "sample_weights": sample_weights,
        "rule_scores": np.asarray(rule_scores, dtype=np.float32),
    }
    _write_npz_atomic(output_paths["dataset"], **arrays)
    partition_counts = Counter(str(value) for value in arrays["partitions"])
    partition_class_counts = {
        partition: {
            "negative": int(
                np.sum(
                    (arrays["partitions"] == partition) & (arrays["labels"] == 0)
                )
            ),
            "positive": int(
                np.sum(
                    (arrays["partitions"] == partition) & (arrays["labels"] == 1)
                )
            ),
        }
        for partition in sorted(DEVELOPMENT_PARTITIONS)
    }
    source_statistics = {
        partition: {
            "event_count": len(
                {
                    str(sample["event_id"])
                    for sample in samples
                    if sample["partition"] == partition
                }
            ),
            "split_group_count": len(
                {
                    str(sample["split_group_id"])
                    for sample in samples
                    if sample["partition"] == partition
                }
            ),
            "source_group_count": len(
                {
                    str(sample["source_group_id"])
                    for sample in samples
                    if sample["partition"] == partition
                }
            ),
            "dataset_counts": dict(
                sorted(
                    Counter(
                        str(sample["dataset"])
                        for sample in samples
                        if sample["partition"] == partition
                    ).items()
                )
            ),
            "action_counts": dict(
                sorted(
                    Counter(
                        str(sample["action_id"])
                        for sample in samples
                        if sample["partition"] == partition
                    ).items()
                )
            ),
        }
        for partition in sorted(DEVELOPMENT_PARTITIONS)
    }
    quality_statistics = {
        name: _distribution_summary(
            [float(sample["quality"][name]) for sample in samples]
        )
        for name in (
            "usable_frame_ratio",
            "core_joint_coverage",
            "mean_joint_quality",
            "interpolated_point_ratio",
            "jump_outlier_per_frame",
        )
    }
    split_payload = {
        "schema_version": "sit-stand-provisional-split-v1",
        "task": "sit_stand_event_presence_proxy_v1",
        "status": "provisional",
        "source_split_id": audit["source_split_id"],
        "source_split_sha256": _sha256_file(split_path),
        "assignments_sha256": _sha256_file(output_paths["assignments"]),
        "protection_fields": [
            "subject_id",
            "physical_event_id",
            "source_group_id",
            "sample_group_id",
            "video_id",
            "split_group_id",
        ],
        "leakage_issues": [],
        "test_policy": "labels_counted_but_pose_and_features_not_read",
        "development_window_counts": dict(sorted(partition_counts.items())),
        "development_class_counts": partition_class_counts,
        "source_statistics": source_statistics,
        "quality_statistics": quality_statistics,
        "locked_test_label_count": len(locked_test_labels),
    }
    _write_json_atomic(output_paths["split"], split_payload)
    metadata = {
        "schema_version": "sit-stand-event-presence-dataset-v1",
        "task": "sit_stand_event_presence_proxy_v1",
        "target_task": "sit_stand_event_localization_v1",
        "status": "provisional",
        "target_semantics": (
            "clip-level sit-stand event presence versus explicitly labelled hard negatives"
        ),
        "label_mapping": {"explicit_hard_negative": 0, "sit_stand_event": 1},
        "positive_action_ids": list(POSITIVE_ACTION_IDS),
        "negative_action_ids": list(EXPLICIT_HARD_NEGATIVE_ACTION_IDS),
        "joint_order": list(SIT_STAND_JOINTS),
        "channel_order": list(SIT_STAND_CHANNELS),
        "tabular_feature_names": list(SIT_STAND_TABULAR_FEATURES),
        "feature_shape": list(arrays["features"].shape),
        "preparation_config": asdict(preparation),
        "normalization_contract": (
            "pelvis_centered_torso_scaled_xy_with_image_y_preserved_and_per_second_velocity"
        ),
        "sample_count": len(samples),
        "event_count": len(event_window_counts),
        "development_window_counts": dict(sorted(partition_counts.items())),
        "development_class_counts": partition_class_counts,
        "source_statistics": source_statistics,
        "quality_statistics": quality_statistics,
        "locked_test_label_count": len(locked_test_labels),
        "test_pose_read": False,
        "rejected_window_counts": dict(sorted(rejected.items())),
        "event_weight_sums": {
            event_id: float(sample_weights[arrays["event_ids"] == event_id].sum())
            for event_id in sorted(event_window_counts)
        },
        "input_sha256": dict(audit["input_sha256"]),
        "audit_sha256": _sha256_file(output_paths["audit"]),
        "source_split_id": audit["source_split_id"],
        "derived_split_sha256": _sha256_file(output_paths["split"]),
        "derived_assignments_sha256": _sha256_file(output_paths["assignments"]),
        "samples_sha256": _sha256_file(output_paths["samples"]),
        "pose_inputs": {
            video_id: {"path": path.as_posix(), "sha256": _sha256_file(path)}
            for video_id, path in sorted(pose_paths.items())
        },
        "limitations": [
            "this is a provisional clip-presence proxy, not temporal event localization",
            "no test pose or test metric is available in this dataset",
            "action clip boundaries are not treated as verified event onset/offset",
            "probabilities are not clinical functional scores or future fall probabilities",
        ],
    }
    metadata["dataset_sha256"] = _sha256_file(output_paths["dataset"])
    _write_json_atomic(output_paths["metadata"], metadata)
    return {
        "output_dir": destination.as_posix(),
        "dataset_path": output_paths["dataset"].as_posix(),
        "metadata_path": output_paths["metadata"].as_posix(),
        "audit_path": output_paths["audit"].as_posix(),
        "split_path": output_paths["split"].as_posix(),
        "sample_count": len(samples),
        "event_count": len(event_window_counts),
        "locked_test_label_count": len(locked_test_labels),
    }


def _derive_center_joint(
    xy: np.ndarray,
    quality: np.ndarray,
    valid: np.ndarray,
    frame_index: int,
    left_index: int,
    right_index: int,
    output_index: int,
) -> None:
    if valid[frame_index, left_index] <= 0 or valid[frame_index, right_index] <= 0:
        return
    xy[frame_index, output_index] = (
        xy[frame_index, left_index] + xy[frame_index, right_index]
    ) / 2.0
    quality[frame_index, output_index] = min(
        quality[frame_index, left_index], quality[frame_index, right_index]
    )
    valid[frame_index, output_index] = 1.0


def _validate_label_rows(rows: Sequence[Mapping[str, Any]]) -> None:
    seen: set[str] = set()
    for row in rows:
        if row.get("schema_version") != "fall-risk-action-label-v3":
            raise ValueError("unsupported sit-stand action label schema")
        label_id = str(row.get("label_id", ""))
        if not label_id or label_id in seen:
            raise ValueError(f"duplicate or missing sit-stand label_id: {label_id}")
        seen.add(label_id)
        tier = row.get("action_type_training_tier")
        if tier not in ALLOWED_ACTION_TIERS:
            raise ValueError(
                f"invalid action_type_training_tier for {label_id}: {tier}"
            )
        if not isinstance(row.get("start_frame"), int):
            raise ValueError(f"invalid start_frame for {label_id}")
        end_frame = row.get("end_frame_exclusive")
        if not isinstance(end_frame, int) or end_frame <= int(row["start_frame"]):
            raise ValueError(f"invalid end_frame_exclusive for {label_id}")
        start_time = _optional_float(row.get("start_time"))
        end_time = _optional_float(row.get("end_time_exclusive"))
        if start_time is None or end_time is None or end_time <= start_time:
            raise ValueError(f"invalid time boundary for {label_id}")


def _assignment_index(rows: Sequence[Mapping[str, Any]]) -> dict[str, dict[str, Any]]:
    index: dict[str, dict[str, Any]] = {}
    for source in rows:
        if source.get("label_kind") != "action":
            continue
        label_id = str(source.get("label_id", ""))
        if not label_id:
            continue
        if label_id in index:
            raise ValueError(f"duplicate sit-stand assignment: {label_id}")
        partition = source.get("partition")
        if partition not in ALL_PARTITIONS:
            raise ValueError(f"invalid sit-stand partition for {label_id}: {partition}")
        if not str(source.get("split_group_id", "")):
            raise ValueError(f"missing sit-stand split_group_id for {label_id}")
        index[label_id] = dict(source)
    return index


def _validate_assignment_links(
    labels: Sequence[Mapping[str, Any]],
    assignments: Mapping[str, Mapping[str, Any]],
) -> None:
    for label in labels:
        label_id = str(label["label_id"])
        assignment = assignments.get(label_id)
        if assignment is None:
            raise ValueError(f"missing sit-stand split assignment for {label_id}")
        for field in (
            "asset_id",
            "video_id",
            "source_group_id",
            "sample_group_id",
            "training_tier",
        ):
            if str(assignment.get(field, "")) != str(label.get(field, "")):
                raise ValueError(f"sit-stand assignment {field} mismatch for {label_id}")


def _protection_unit_leakage(
    labels: Sequence[Mapping[str, Any]],
    assignments: Mapping[str, Mapping[str, Any]],
) -> list[dict[str, Any]]:
    values: dict[tuple[str, str], set[str]] = defaultdict(set)
    for label in labels:
        assignment = assignments[str(label["label_id"])]
        partition = str(assignment["partition"])
        fields = {
            "subject_id": label.get("subject_id"),
            "physical_event_id": assignment.get("physical_event_id"),
            "source_group_id": label.get("source_group_id"),
            "sample_group_id": label.get("sample_group_id"),
            "video_id": label.get("video_id"),
            "split_group_id": assignment.get("split_group_id"),
        }
        for field, raw_value in fields.items():
            value = str(raw_value or "").strip()
            if not value or value.lower() in {"unknown", "none", "null"}:
                continue
            values[(field, value)].add(partition)
    return [
        {"field": field, "value": value, "partitions": sorted(partitions)}
        for (field, value), partitions in sorted(values.items())
        if len(partitions) > 1
    ]


def _validate_input_hashes(
    *,
    labels_path: Path,
    manifest_path: Path,
    assignments_path: Path,
    split_path: Path,
    split_report: Mapping[str, Any],
    validation_report: Mapping[str, Any],
) -> None:
    if split_report.get("schema_version") != "fall-risk-training-split-v3":
        raise ValueError("unsupported sit-stand source split schema")
    split_hashes = split_report.get("input_sha256", {})
    expected = {
        "action_labels": _sha256_file(labels_path),
        "manifest": _sha256_file(manifest_path),
    }
    for key, digest in expected.items():
        if split_hashes.get(key) != digest:
            raise ValueError(f"sit-stand source split {key} SHA-256 mismatch")
    if split_report.get("assignments_sha256") != _sha256_file(assignments_path):
        raise ValueError("sit-stand source assignments SHA-256 mismatch")
    validation_hashes = validation_report.get("input_sha256", {})
    optional_expected = {
        "action_labels": expected["action_labels"],
        "manifest": expected["manifest"],
        "split_assignments": _sha256_file(assignments_path),
        "split_report": _sha256_file(split_path),
    }
    for key, digest in optional_expected.items():
        if key in validation_hashes and validation_hashes.get(key) != digest:
            raise ValueError(f"sit-stand validation {key} SHA-256 mismatch")
    if validation_report.get("valid") is not True:
        raise ValueError("sit-stand v3 validation report is not structurally valid")


def _manifest_index(rows: Sequence[Mapping[str, Any]]) -> dict[str, dict[str, Any]]:
    index: dict[str, dict[str, Any]] = {}
    for source in rows:
        raw_video_id = source.get("video_id")
        if raw_video_id in (None, ""):
            continue
        video_id = str(raw_video_id)
        if video_id in index:
            raise ValueError(f"duplicate sit-stand manifest video_id: {video_id}")
        index[video_id] = dict(source)
    return index


def _action_summary(
    rows: Sequence[Mapping[str, Any]],
    assignments: Mapping[str, Mapping[str, Any]],
    manifests: Mapping[str, Mapping[str, Any]],
    pose_root: Path,
) -> dict[str, Any]:
    tier_counts = Counter(str(row["action_type_training_tier"]) for row in rows)
    partition_counts = Counter(
        str(assignments[str(row["label_id"])]["partition"]) for row in rows
    )
    primary_partition_counts = Counter(
        str(assignments[str(row["label_id"])]["partition"])
        for row in rows
        if row["action_type_training_tier"] == "primary"
    )
    boundary_counts = Counter(str(row.get("boundary_precision", "unknown")) for row in rows)
    datasets = Counter(
        str(manifests.get(str(row["video_id"]), {}).get("dataset", "missing"))
        for row in rows
    )
    media_found = 0
    pose_found = 0
    for row in rows:
        video_id = str(row["video_id"])
        manifest = manifests.get(video_id)
        if manifest is not None:
            path = Path(str(manifest.get("path", "")))
            if path.is_file():
                media_found += 1
        if _resolve_pose_path(pose_root, video_id) is not None:
            pose_found += 1
    return {
        "total": len(rows),
        "action_type_tier_counts": dict(sorted(tier_counts.items())),
        "partition_counts": dict(sorted(partition_counts.items())),
        "primary_partition_counts": dict(sorted(primary_partition_counts.items())),
        "boundary_precision_counts": dict(sorted(boundary_counts.items())),
        "subject_count": len(
            {str(row.get("subject_id")) for row in rows if row.get("subject_id")}
        ),
        "source_group_count": len({str(row["source_group_id"]) for row in rows}),
        "sample_group_count": len({str(row["sample_group_id"]) for row in rows}),
        "video_count": len({str(row["video_id"]) for row in rows}),
        "dataset_counts": dict(sorted(datasets.items())),
        "media_path_found_count": media_found,
        "pose_path_found_count": pose_found,
    }


def _is_explicit_background(row: Mapping[str, Any]) -> bool:
    if row.get("action_type") == "background":
        return True
    attributes = row.get("action_attributes", [])
    return isinstance(attributes, Sequence) and "explicit_background" in attributes


def _proxy_partition_counts(
    rows: Sequence[Mapping[str, Any]],
    assignments: Mapping[str, Mapping[str, Any]],
) -> dict[str, dict[str, int]]:
    result = {
        partition: {"positive": 0, "negative": 0}
        for partition in ("train", "validation", "test")
    }
    for row in rows:
        partition = str(assignments[str(row["label_id"])]["partition"])
        target = "positive" if row["action_id"] in POSITIVE_ACTION_IDS else "negative"
        result[partition][target] += 1
    return result


def _missing_development_pose_ids(
    rows: Sequence[Mapping[str, Any]],
    assignments: Mapping[str, Mapping[str, Any]],
    pose_root: Path,
) -> list[str]:
    return sorted({
        str(row["video_id"])
        for row in rows
        if assignments[str(row["label_id"])]["partition"] in DEVELOPMENT_PARTITIONS
        and _resolve_pose_path(pose_root, str(row["video_id"])) is None
    })


def _resolve_pose_path(root: Path, video_id: str) -> Path | None:
    candidates = (
        root / f"{video_id}.jsonl",
        root / f"{video_id}_poses_cleaned.jsonl",
    )
    return next((path for path in candidates if path.is_file()), None)


def _select_labeled_track(
    label: Mapping[str, Any], records: Sequence[Mapping[str, Any]]
) -> dict[int, dict[str, Any]]:
    start_frame = int(label["start_frame"])
    end_frame = int(label["end_frame_exclusive"]) - 1
    by_track: dict[str, dict[int, dict[str, Any]]] = defaultdict(dict)
    for source in records:
        frame_id = int(source.get("frame_id", -1))
        if not start_frame <= frame_id <= end_frame:
            continue
        track_id = str(source.get("track_id", source.get("person_id", "unknown")))
        previous = by_track[track_id].get(frame_id)
        record = dict(source)
        if previous is None or _record_confidence(record) > _record_confidence(previous):
            by_track[track_id][frame_id] = record
    if not by_track:
        return {}
    return max(
        by_track.values(),
        key=lambda track: (
            len(track),
            float(np.mean([_record_confidence(row) for row in track.values()])),
        ),
    )


def _record_confidence(record: Mapping[str, Any]) -> float:
    value = _optional_float(record.get("pose_confidence", record.get("keypoint_quality")))
    return value if value is not None else 0.0


def _window_start_times(
    start_time: float, end_time: float, config: SitStandDatasetConfig
) -> list[float]:
    duration = end_time - start_time
    if duration <= config.window_sec:
        return [start_time]
    final_start = end_time - config.window_sec
    values = list(
        np.arange(start_time, final_start + 1e-9, config.stride_sec, dtype=np.float64)
    )
    if not values or abs(values[-1] - final_start) > 1e-6:
        values.append(final_start)
    if len(values) > config.max_windows_per_event:
        indices = np.linspace(
            0, len(values) - 1, config.max_windows_per_event, dtype=np.int64
        )
        values = [values[index] for index in sorted(set(indices.tolist()))]
    return [float(value) for value in values]


def _resample_pose_records(
    records: Sequence[Mapping[str, Any]],
    *,
    start_time_sec: float,
    window_frames: int,
    target_fps: float,
    max_gap_sec: float,
) -> list[dict[str, Any] | None]:
    timed = sorted(
        (
            (float(row["timestamp_sec"]), dict(row))
            for row in records
            if _optional_float(row.get("timestamp_sec")) is not None
        ),
        key=lambda item: item[0],
    )
    selected: dict[int, tuple[float, int]] = {}
    for source_index, (source_time, _) in enumerate(timed):
        target_index = int(np.floor(((source_time - start_time_sec) * target_fps) + 0.5))
        if not 0 <= target_index < window_frames:
            continue
        target_time = start_time_sec + (target_index / target_fps)
        distance = abs(source_time - target_time)
        if distance > max_gap_sec:
            continue
        candidate = (distance, source_index)
        if target_index not in selected or candidate < selected[target_index]:
            selected[target_index] = candidate
    output: list[dict[str, Any] | None] = []
    for target_index in range(window_frames):
        candidate = selected.get(target_index)
        if candidate is None:
            output.append(None)
            continue
        row = dict(timed[candidate[1]][1])
        row["timestamp_sec"] = round(
            start_time_sec + (target_index / target_fps), 6
        )
        output.append(row)
    return output


def _window_quality(
    records: Sequence[Mapping[str, Any] | None],
) -> dict[str, float]:
    observed = [record for record in records if record is not None]
    if not observed:
        return {
            "usable_frame_ratio": 0.0,
            "core_joint_coverage": 0.0,
            "mean_joint_quality": 0.0,
            "interpolated_point_ratio": 0.0,
            "jump_outlier_per_frame": 0.0,
        }
    usable = sum(
        record.get("window_quality", {}).get("usable_for_sit_stand") is True
        for record in observed
    )
    core_names = {
        "left_shoulder",
        "right_shoulder",
        "left_hip",
        "right_hip",
        "left_knee",
        "right_knee",
        "left_ankle",
        "right_ankle",
    }
    valid_core = 0
    qualities: list[float] = []
    interpolated = 0
    jumps = 0
    for record in observed:
        for point in record.get("keypoints", []):
            if not isinstance(point, Mapping) or point.get("name") not in core_names:
                continue
            if point.get("source") == "interpolated":
                interpolated += 1
            if point.get("is_jump_outlier") is True:
                jumps += 1
            if point.get("valid") is True and point.get("is_jump_outlier") is not True:
                valid_core += 1
                quality = _optional_float(
                    point.get("quality_weight", point.get("score"))
                )
                if quality is not None:
                    qualities.append(quality)
    denominator = max(1, len(observed) * len(core_names))
    return {
        "usable_frame_ratio": usable / len(observed),
        "core_joint_coverage": valid_core / denominator,
        "mean_joint_quality": float(np.mean(qualities)) if qualities else 0.0,
        "interpolated_point_ratio": interpolated / denominator,
        "jump_outlier_per_frame": jumps / len(observed),
    }


def _tabular_features(
    tensor: np.ndarray,
    records: Sequence[Mapping[str, Any] | None],
    quality: Mapping[str, float],
) -> list[float]:
    valid = tensor[..., 6] > 0
    pelvis_mask = valid[:, 12]
    shoulder_mask = valid[:, 13]
    pelvis_y = tensor[pelvis_mask, 12, 5]
    shoulder_y = tensor[shoulder_mask, 13, 5]
    pelvis_x = tensor[pelvis_mask, 12, 0]
    pelvis_vy = np.abs(tensor[pelvis_mask, 12, 3])
    leg_extension: list[float] = []
    trunk_angles: list[float] = []
    for frame_index in range(len(tensor)):
        if valid[frame_index, 12] and valid[frame_index, 13]:
            shoulder = tensor[frame_index, 13, :2]
            pelvis = tensor[frame_index, 12, :2]
            vector = shoulder - pelvis
            trunk_angles.append(
                float(np.degrees(np.arctan2(abs(vector[0]), max(abs(vector[1]), 1e-6))))
            )
        if valid[frame_index, 12] and valid[frame_index, 10] and valid[frame_index, 11]:
            ankle_center = (
                tensor[frame_index, 10, :2] + tensor[frame_index, 11, :2]
            ) / 2.0
            leg_extension.append(
                float(np.linalg.norm(ankle_center - tensor[frame_index, 12, :2]))
            )
    values = [
        _range(pelvis_y),
        _net_change(pelvis_y),
        _range(shoulder_y),
        _net_change(shoulder_y),
        _range(np.asarray(leg_extension, dtype=np.float32)),
        _percentile(np.asarray(trunk_angles, dtype=np.float32), 0.90),
        float(np.std(pelvis_x)) if len(pelvis_x) else 0.0,
        _percentile(pelvis_vy, 0.90),
        float(quality["usable_frame_ratio"]),
        float(quality["core_joint_coverage"]),
        float(quality["mean_joint_quality"]),
        float(quality["interpolated_point_ratio"]),
        float(quality["jump_outlier_per_frame"]),
    ]
    if len(values) != len(SIT_STAND_TABULAR_FEATURES) or not np.isfinite(values).all():
        raise ValueError("invalid sit-stand tabular feature vector")
    return [float(value) for value in values]


def _rule_prediction(records: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    events = extract_sit_stand_events(records)
    candidates = [
        event
        for event in events
        if event.get("transition_type") in {"sit_to_stand", "stand_to_sit"}
        and not event.get("quality_coverage", {}).get("insufficient_sit_stand_quality")
    ]
    if not candidates:
        return {"detected": False, "direction": "none"}
    best = max(candidates, key=lambda row: float(row.get("duration", 0.0) or 0.0))
    return {"detected": True, "direction": str(best["transition_type"])}


def _validate_development_samples(samples: Sequence[Mapping[str, Any]]) -> None:
    for partition in sorted(DEVELOPMENT_PARTITIONS):
        targets = {
            int(sample["label"])
            for sample in samples
            if sample["partition"] == partition
        }
        if targets != {0, 1}:
            raise ValueError(f"sit-stand {partition} partition lacks a binary class")
    for field in (
        "subject_id",
        "source_group_id",
        "sample_group_id",
        "video_id",
        "split_group_id",
        "event_id",
    ):
        partitions: dict[str, set[str]] = defaultdict(set)
        for sample in samples:
            value = str(sample.get(field, "")).strip()
            if not value or value.lower() == "unknown":
                continue
            partitions[value].add(str(sample["partition"]))
        leaked = sorted(value for value, parts in partitions.items() if len(parts) > 1)
        if leaked:
            raise ValueError(f"sit-stand {field} values cross partitions: {leaked[:5]}")


def _validate_event_weights(
    samples: Sequence[Mapping[str, Any]], sample_weights: np.ndarray
) -> None:
    event_ids = np.asarray([str(sample["event_id"]) for sample in samples])
    for event_id in np.unique(event_ids):
        total = float(sample_weights[event_ids == event_id].sum())
        if not np.isclose(total, 1.0, atol=1e-6):
            raise ValueError(f"sit-stand event weights do not sum to 1: {event_id}")


def _range(values: np.ndarray) -> float:
    return float(np.ptp(values)) if len(values) else 0.0


def _net_change(values: np.ndarray) -> float:
    return float(values[-1] - values[0]) if len(values) >= 2 else 0.0


def _percentile(values: np.ndarray, quantile: float) -> float:
    return float(np.quantile(values, quantile)) if len(values) else 0.0


def _distribution_summary(values: Sequence[float]) -> dict[str, float]:
    array = np.asarray(values, dtype=np.float64)
    if len(array) == 0 or not np.isfinite(array).all():
        raise ValueError("sit-stand quality statistics require finite values")
    return {
        "min": float(np.min(array)),
        "p10": float(np.quantile(array, 0.10)),
        "median": float(np.median(array)),
        "mean": float(np.mean(array)),
        "p90": float(np.quantile(array, 0.90)),
        "max": float(np.max(array)),
    }


def _optional_float(value: Any) -> float | None:
    try:
        if value is None:
            return None
        result = float(value)
    except (TypeError, ValueError):
        return None
    return result if np.isfinite(result) else None


def _read_jsonl(path: Path) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    with path.open("r", encoding="utf-8") as file:
        for line_number, line in enumerate(file, start=1):
            stripped = line.strip()
            if not stripped:
                continue
            value = json.loads(stripped)
            if not isinstance(value, dict):
                raise ValueError(f"{path}:{line_number} must contain a JSON object")
            rows.append(value)
    return rows


def _sha256_file(path: Path) -> str:
    digest = hashlib.sha256()
    with path.open("rb") as file:
        for chunk in iter(lambda: file.read(1024 * 1024), b""):
            digest.update(chunk)
    return digest.hexdigest()


def _write_npz_atomic(path: Path, **arrays: np.ndarray) -> None:
    partial = path.with_suffix(f"{path.suffix}.part")
    with partial.open("wb") as file:
        np.savez_compressed(file, **arrays)
    os.replace(partial, path)


def _write_jsonl_atomic(path: Path, rows: Iterable[Mapping[str, Any]]) -> None:
    partial = path.with_suffix(f"{path.suffix}.part")
    with partial.open("w", encoding="utf-8") as file:
        for row in rows:
            file.write(json.dumps(dict(row), ensure_ascii=False, sort_keys=True) + "\n")
    os.replace(partial, path)


def _write_json_atomic(path: Path, payload: Mapping[str, Any]) -> None:
    partial = path.with_suffix(f"{path.suffix}.part")
    partial.write_text(
        json.dumps(dict(payload), ensure_ascii=False, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    os.replace(partial, path)
