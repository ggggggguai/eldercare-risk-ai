"""Leakage-safe V3.3.3 ActivityExpert training and inference artifacts."""
...
TRAINING_CONFIG_VERSION = "mood-social-activity-training-config-v1"
MODEL_BUNDLE_VERSION = "mood-social-activity-expert-bundle-v1"
...
PREDICTIONS_VERSION = "mood-social-activity-oof-predictions-v1"
RUN_MANIFEST_VERSION = "mood-social-activity-training-run-v1"
MODEL_MANIFEST_VERSION = "mood-social-activity-model-manifest-v1"
...
ACTIVITY_DATASET_IDS = ("psyche_d", "resilient", "nhanes")
EXCLUDED_DATASET_IDS = ("shenzhen_elderly", "nhanes_ssq_2005_2008")
ACTIVITY_FEATURE_COLUMNS = tuple(
    f"activity.{name}" for name in ACTIVITY_EXPERT_FEATURE_ORDER
)
ACTIVITY_MASK_COLUMNS = tuple(
    f"feature_mask.{name}" for name in ACTIVITY_FEATURE_COLUMNS
)
OUTER_FOLD_COUNT = 5
INNER_FOLD_COUNT = 5
...
class ActivityExpertError(RuntimeError):
    """Raised when MODEL-001 input, training, or artifact checks fail."""


@dataclass(frozen=True)
class ActivityTrainingConfig:
    run_id: str
    random_seed: int
    split_id: str
    split_sha256: str
...
class NumericFoldPreprocessor:
    ...
    def __post_init__(self) -> None:
        if self.version != PREPROCESSOR_VERSION:
            raise ActivityExpertError("unsupported ActivityExpert preprocessor")
        if self.input_features != ACTIVITY_FEATURE_COLUMNS:
            raise ActivityExpertError("ActivityExpert input feature order changed")
        if not self.selected_features:
            raise ActivityExpertError("ActivityExpert selected no usable features")
        if not set(self.selected_features).issubset(self.input_features):
            raise ActivityExpertError("ActivityExpert selected an unknown feature")
        if len(self.selected_features) != len(self.medians):
            raise ActivityExpertError("ActivityExpert median count does not match")
        if len(self.input_features) != len(self.observed_counts) or len(
            self.input_features
        ) != len(self.unique_counts):
            raise ActivityExpertError("ActivityExpert selection audit does not match")
        if any(not math.isfinite(value) for value in self.medians):
            raise ActivityExpertError("ActivityExpert medians must be finite")
        if self.add_missing_indicators:
            raise ActivityExpertError("feature masks cannot become learned risk inputs")

    @classmethod
    def fit(cls, frame: pd.DataFrame) -> NumericFoldPreprocessor:
        values, _ = _masked_activity_values(frame)
        selected: list[str] = []
        medians: list[float] = []
        observed_counts: list[int] = []
        unique_counts: list[int] = []
        for index, feature in enumerate(ACTIVITY_FEATURE_COLUMNS):
        ...
        return cls(
            input_features=ACTIVITY_FEATURE_COLUMNS,
    ...
    def transform(self, frame: pd.DataFrame) -> pd.DataFrame:
        ...
        if not np.isfinite(selected).all():
            raise ActivityExpertError(
                "ActivityExpert preprocessing left non-finite data"
            )
        return pd.DataFrame(
            selected,
...
class ProbabilityCalibrator:
    ...
    def __post_init__(self) -> None:
        if self.version != CALIBRATOR_VERSION:
            raise ActivityExpertError("unsupported probability calibrator")
        if self.method not in {"isotonic", "platt"}:
            raise ActivityExpertError("unsupported calibration method")
        if self.positive_participant_count <= 0 or self.fit_row_count <= 0:
            raise ActivityExpertError("calibrator training scope is empty")
    ...
    def fit(
        ...
        *,
        ...
        if (
            ...
            raise ActivityExpertError("calibrator inputs are invalid")
        method = (
            "isotonic"
    ...
    def predict(self, raw_probability: Sequence[float]) -> np.ndarray:
        ...
        if raw.ndim != 1 or not np.isfinite(raw).all():
            raise ActivityExpertError("calibrator prediction input is invalid")
...
class ActivityExpertBundle:
    ...
    preprocessor: NumericFoldPreprocessor
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ActivityExpertError("ActivityExpert bundle identity changed")
        if self.feature_schema_version != FEATURE_SCHEMA_VERSION:
            raise ActivityExpertError("ActivityExpert feature schema changed")
        if self.feature_schema_sha256 != EXPECTED_FEATURE_SCHEMA_SHA256:
            raise ActivityExpertError("ActivityExpert feature schema hash changed")
        if self.split_id != SPLIT_ID:
            raise ActivityExpertError("ActivityExpert split ID changed")
        if self.split_manifest_sha256 != EXPECTED_SPLIT_SHA256:
            raise ActivityExpertError("ActivityExpert split hash changed")
        if self.dataset_ids != ACTIVITY_DATASET_IDS:
            raise ActivityExpertError("ActivityExpert dataset membership changed")
        if self.input_feature_names != ACTIVITY_FEATURE_COLUMNS:
            raise ActivityExpertError("ActivityExpert feature order changed")
        if set(self.ecdf_by_dataset) != set(ACTIVITY_DATASET_IDS):
            raise ActivityExpertError("ActivityExpert ECDF membership changed")
        if self.training_participant_count <= 0 or self.training_row_count <= 0:
            raise ActivityExpertError("ActivityExpert training scope is empty")
        if not _is_sha256(self.training_participant_sha256):
            raise ActivityExpertError("ActivityExpert participant hash is invalid")

    def predict(self, frame: pd.DataFrame) -> pd.DataFrame:
        values, masks = _masked_activity_values(frame)
...
def load_activity_training_config(path: str | Path) -> ActivityTrainingConfig:
    config_path = Path(path).resolve()
    try:
        payload = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    except (OSError, UnicodeError, yaml.YAMLError) as exc:
        raise ActivityExpertError(
            "ActivityExpert training config is unreadable"
        ) from exc
    if not isinstance(payload, dict):
        raise ActivityExpertError("ActivityExpert training config must be a mapping")
    try:
        search_space_payload = payload["search"]["space"]
        search_space = {
            str(name): tuple(values) for name, values in search_space_payload.items()
        }
        config = ActivityTrainingConfig(
        ...
        raise ActivityExpertError(
            "ActivityExpert training config is malformed"
        ) from exc
...
def _validate_training_config(
    ...
    if payload.get("config_version") != TRAINING_CONFIG_VERSION:
        raise ActivityExpertError("ActivityExpert config version changed")
    if payload.get("task_id") != "MODEL-001" or payload.get("model_id") != MODEL_ID:
        raise ActivityExpertError("ActivityExpert config task identity changed")
    if payload.get("model_version") != MODEL_VERSION:
        raise ActivityExpertError("ActivityExpert config model version changed")
    if (
        ...
        or config.dataset_ids != ACTIVITY_DATASET_IDS
    ):
        raise ActivityExpertError("ActivityExpert frozen input binding changed")
    expected_space = {
    ...
    if dict(config.search_space) != expected_space:
        raise ActivityExpertError("ActivityExpert LightGBM search space changed")
    ...
    if (
        ...
        raise ActivityExpertError("ActivityExpert implementation settings are invalid")
    ...
    if preprocessing != {
        ...
        raise ActivityExpertError("ActivityExpert preprocessing policy changed")
...
def deterministic_search_candidates(
    ...
    *,
    ...
    if not 1 <= candidate_count <= len(candidates):
        raise ActivityExpertError("search candidate count is outside the grid")
...
def compute_training_sample_weights(frame: pd.DataFrame) -> np.ndarray:
    ...
    if not required.issubset(frame.columns) or len(frame) == 0:
        raise ActivityExpertError("sample-weight frame is incomplete")
    ...
    if not target.isin([0, 1]).all():
        raise ActivityExpertError("sample-weight target must be binary")
    ...
    if not np.isfinite(weights).all() or np.any(weights <= 0):
        raise ActivityExpertError("sample-weight calculation failed")
...
def sample_weight_audit(
    ...
    if len(weight) != len(frame):
        raise ActivityExpertError("sample-weight audit length does not match")
    rows: dict[str, Any] = {}
...
def binary_metrics(
    ...
    *,
    ...
    if (
        ...
        raise ActivityExpertError("binary metric inputs are invalid")
...
def load_activity_training_inputs(
    ...
    *,
    ...
    if _sha256_file(split_path) != expected_split_sha256:
        raise ActivityExpertError("frozen DATA-007 split file hash changed")
    try:
        manifest = json.loads(split_path.read_text(encoding="utf-8"))
    except (OSError, UnicodeError, json.JSONDecodeError) as exc:
        raise ActivityExpertError("frozen DATA-007 split is unreadable") from exc
    rebuilt = build_split_manifest(repository_root=root)
    if manifest != rebuilt:
        raise ActivityExpertError("split, five inputs, or MH-003 schema drifted")
    if (
        manifest.get("split_id") != SPLIT_ID
        or manifest.get("feature_schema_binding", {}).get("snapshot_sha256")
        != EXPECTED_FEATURE_SCHEMA_SHA256
    ):
        raise ActivityExpertError("frozen DATA-007 binding changed")
    inputs = manifest.get("inputs")
    if not isinstance(inputs, list):
        raise ActivityExpertError("frozen DATA-007 inputs are malformed")
    input_bindings = {
        str(item["dataset_id"]): item for item in inputs if isinstance(item, dict)
    }
    if set(input_bindings) != set(ACTIVITY_DATASET_IDS + EXCLUDED_DATASET_IDS):
        raise ActivityExpertError("frozen DATA-007 dataset membership changed")
    ...
    for dataset_id in ACTIVITY_DATASET_IDS:
        ...
        if _sha256_file(path) != binding["canonical_sha256"]:
            raise ActivityExpertError("ActivityExpert canonical hash changed")
        try:
            frame = pd.read_parquet(path)
        except (OSError, TypeError, ValueError) as exc:
            raise ActivityExpertError("ActivityExpert canonical is unreadable") from exc
        if not isinstance(frame.index, pd.RangeIndex):
            frame = frame.reset_index(drop=True)
        _validate_base_activity_frame(frame, dataset_id, binding)
        frames[dataset_id] = frame
    assignments = pd.DataFrame(manifest["participant_assignments"])
    assignments = assignments[
        assignments["dataset_id"].astype(str).isin(ACTIVITY_DATASET_IDS)
    ].copy()
    ...
    if set(assignments["global_participant_id"].astype(str)) != expected_keys:
        raise ActivityExpertError(
            "ActivityExpert split does not cover its participants"
        )
    return ActivityTrainingInputs(
...
def train_activity_expert(
    ...
    *,
    ...
    root = Path(repository_root).resolve()
    inputs = load_activity_training_inputs(
        root,
    ...
    production_manifest = (
        ...
        else production_model.with_name("activity_expert_manifest.json")
    )
...
def _train_into_stage(
    ...
    *,
    ...
    model_manifest = {
        ...
        "dataset_ids": list(ACTIVITY_DATASET_IDS),
        "excluded_dataset_ids": list(EXCLUDED_DATASET_IDS),
        "input_feature_names": list(ACTIVITY_FEATURE_COLUMNS),
...
def _prepare_inner_fold(
    ...
    *,
    ...
    if not train_keys or not validation_keys or train_keys & validation_keys:
        raise ActivityExpertError("inner fold participant relation is invalid")
    ecdf = _fit_ecdfs(inputs, train_keys)
    train_table = _transform_partition(inputs, ecdf, train_keys)
    validation_table = _transform_partition(inputs, ecdf, validation_keys)
    train_table = _available_activity_rows(train_table)
    validation_table = _available_activity_rows(validation_table)
    preprocessor = NumericFoldPreprocessor.fit(train_table)
...
def _fit_outer_fold(
    ...
    *,
    ...
    bundle = ActivityExpertBundle(
        ...
        input_feature_names=ACTIVITY_FEATURE_COLUMNS,
        preprocessor=preprocessor,
...
def _fit_final_bundle(
    ...
    *,
    ...
    hyperparameters: Mapping[str, int | float],
    config: ActivityTrainingConfig,
) -> tuple[ActivityExpertBundle, dict[str, Any]]:
    assignments = _assignment_lookup(inputs.assignments)
    ...
    if (
        ...
        raise ActivityExpertError("production calibrator OOF coverage is invalid")
    ecdf = _fit_ecdfs(inputs, all_keys)
    partition = _transform_partition(inputs, ecdf, all_keys)
    table = _available_activity_rows(partition)
    preprocessor = NumericFoldPreprocessor.fit(table)
    ...
    bundle = ActivityExpertBundle(
        ...
        ecdf_by_dataset=ecdf,
        hyperparameters=dict(hyperparameters),
        training_scope="all_activity_sources_for_production",
        training_participant_sha256=participant_set_sha256(
...
def _transform_partition(
    ...
    for dataset_order, dataset_id in enumerate(ACTIVITY_DATASET_IDS):
        base = inputs.frames[dataset_id]
        selected = base[
            base["global_participant_id"].astype(str).isin(participant_keys)
        ]
        if selected.empty:
            raise ActivityExpertError("training partition omitted an activity source")
    ...
    _masked_activity_values(result)
    return result


def _available_activity_rows(frame: pd.DataFrame) -> pd.DataFrame:
    _, masks = _masked_activity_values(frame)
    available = masks.sum(axis=1) > 0
    result = frame.loc[available].reset_index(drop=True)
    if result.empty:
        raise ActivityExpertError("ActivityExpert fold has no available feature rows")
    return result
...
def _validate_oof_predictions(
    ...
    if (
        ...
        or set(oof["dataset_id"].astype(str)) != set(ACTIVITY_DATASET_IDS)
    ):
        raise ActivityExpertError("outer OOF prediction coverage is invalid")
    ...
    if (
        ...
        raise ActivityExpertError("outer OOF probability-mask relation is invalid")
    expected_outer = (
    ...
    if not expected_outer.astype("int64").equals(oof["outer_fold"].astype("int64")):
        raise ActivityExpertError("outer OOF prediction fold assignment drifted")


def _validate_base_activity_frame(
    frame: pd.DataFrame,
    dataset_id: str,
    ...
    if not required.issubset(frame.columns):
        raise ActivityExpertError("ActivityExpert canonical columns changed")
    if (
        ...
        raise ActivityExpertError("ActivityExpert canonical summary changed")
    ...
    if (
        ...
        raise ActivityExpertError("ActivityExpert base ECDF contract changed")


def _masked_activity_values(
    frame: pd.DataFrame,
) -> tuple[np.ndarray, np.ndarray]:
    missing_columns = set(ACTIVITY_FEATURE_COLUMNS + ACTIVITY_MASK_COLUMNS) - set(
        frame.columns
    )
    if missing_columns:
        raise ActivityExpertError("ActivityExpert unified feature columns are missing")
    values = np.empty((len(frame), len(ACTIVITY_FEATURE_COLUMNS)), dtype="float64")
    masks = np.empty_like(values, dtype="int8")
    for index, (feature, mask_name) in enumerate(
        zip(ACTIVITY_FEATURE_COLUMNS, ACTIVITY_MASK_COLUMNS, strict=True)
    ):
        try:
            ...
            raise ActivityExpertError(
                "ActivityExpert feature values are invalid"
            ) from exc
        if (
            not np.isfinite(mask_float).all()
            or not np.isin(mask_float, [0.0, 1.0]).all()
        ):
            raise ActivityExpertError("ActivityExpert feature masks are invalid")
        mask = mask_float.astype("int8")
        finite = np.isfinite(value)
        if np.any((mask == 1) & ~finite) or np.any((mask == 0) & finite):
            raise ActivityExpertError("ActivityExpert value-mask relation is invalid")
        values[:, index] = value
        masks[:, index] = mask
    return values, masks
...
def participant_set_sha256(participant_ids: Sequence[str] | set[str]) -> str:
    ...
    if not values or len(values) != len(set(values)):
        raise ActivityExpertError("participant hash input must be unique and non-empty")
    for participant_id in sorted(values, key=lambda value: value.encode("utf-8")):
        if (
            ...
            raise ActivityExpertError("participant hash input is invalid")
...
def _feature_importance(bundle: ActivityExpertBundle) -> dict[str, Any]:
...
def _assert_bundle_mask_diagnostic(bundle: ActivityExpertBundle) -> None:
    frame = pd.DataFrame(
        {
            **{name: [np.nan] for name in ACTIVITY_FEATURE_COLUMNS},
            **{name: [0] for name in ACTIVITY_MASK_COLUMNS},
        }
    )
    ...
    if (
        ...
        raise ActivityExpertError("ActivityExpert all-missing mask diagnostic failed")
...
def _training_warnings(
    ...
    unselected = sorted(
        set(ACTIVITY_FEATURE_COLUMNS) - set(bundle.preprocessor.selected_features)
    )
    return [
        {
            "code": "PROXY_TRANSFER",
            "message": (
                "Public wearable activity is a deployable semantic proxy and is "
                "not numerically equivalent to camera activity."
            ),
        },
        {
        ...
        },
        {
            ...
            "message": (
                "Metrics are reported as observed; MODEL-001 has no minimum "
...
def _run_manifest(
    ...
    *,
    ...
    return {
        ...
        "all_mask_fusion_hard_gate": "not_applicable_to_MODEL_001",
        "failures": [],
        ...
        "production_package_status": (
            "standalone_activity_expert_active_full_v3_3_3_package_pending_ART_001"
        ),
        "conclusion": (
            "ActivityExpert completed with strict nested participant OOF "
...
def _report_artifact_hashes(stage: Path) -> dict[str, Any]:
    ...
    return {
        "artifact_manifest_version": "mood-social-activity-report-artifacts-v1",
...
def _sha256_file(path: Path, chunk_size: int = 1024 * 1024) -> str:
    ...
    try:
        ...
        raise ActivityExpertError("required ActivityExpert file is unreadable") from exc
    return digest.hexdigest()
