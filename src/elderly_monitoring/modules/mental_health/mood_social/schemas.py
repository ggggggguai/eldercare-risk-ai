...
MOOD_SOCIAL_PRODUCTION_MODEL_VERSION = "mood-social-current-state-production-r5"
...
class MoodSocialProfile(MoodSocialSchemaModel):
    ...
    education_level: Literal["primary_or_less", "middle", "high_or_above"] | None = None
...
class MoodSocialActivity(MoodSocialSchemaModel):
    ...
    def validate_observation_semantics(self) -> "MoodSocialActivity":
        ...
        hourly_valid_total = sum(
            (Decimal(str(minutes)) for minutes in self.hourly_valid_detection_minutes),
            start=Decimal("0"),
        )
        ...
        if (
            self.low_activity_minutes is not None
            and self.low_activity_minutes > valid_minutes
        ):
            raise _contract_error(
                "field_validation_error",
        ...
        if self.weighted_daytime_activity is not None:
            hourly_weighted = sum(
                (
                    ...
                    * Decimal(str(intensity if intensity is not None else 0.0))
...
class MoodSocialSleep(MoodSocialSchemaModel):
    ...
    def validate_sleep_semantics(self) -> "MoodSocialSleep":
        ...
        if (
            ...
            expected = Decimal(str(self.sleep_minutes)) / Decimal(
                str(self.in_bed_minutes)
            )
...
class MoodSocialHistoryPHQ9Assessment(MoodSocialSchemaModel):
    date: DateOnly
    known_at: DateOnly | None = None
    score: Annotated[int, Field(strict=True, ge=0, le=27)]
    source: Literal["s10", "mini_program", "clinical_import", "manual"]
...
class MoodSocialInferRequest(MoodSocialSchemaModel):
    ...
    historical_phq9_assessments: Annotated[
        list[MoodSocialHistoryPHQ9Assessment],
        Field(default_factory=list, max_length=36),
    ]
    ...
    def validate_request_relationships(self) -> "MoodSocialInferRequest":
        ...
        attention_dates = [record.date for record in self.history_attention_indices]
        ...
        assessment_dates = [record.date for record in self.historical_phq9_assessments]
        if assessment_dates != sorted(assessment_dates):
            raise _contract_error(
                "date_validation_error",
                "historical PHQ-9 assessments must be ordered by ascending date",
                field="historical_phq9_assessments",
            )
        if len(assessment_dates) != len(set(assessment_dates)):
            raise _contract_error(
                "date_validation_error",
                "historical PHQ-9 assessment dates must not repeat",
                field="historical_phq9_assessments",
            )
        if any(value >= self.target_date for value in assessment_dates):
            raise _contract_error(
                "date_validation_error",
                "historical PHQ-9 assessments must be earlier than target_date",
                field="historical_phq9_assessments",
            )
        if any(
            record.known_at is not None and record.known_at > self.target_date
            for record in self.historical_phq9_assessments
        ):
            raise _contract_error(
                "date_validation_error",
                "historical PHQ-9 assessments must be known by target_date",
                field="historical_phq9_assessments",
            )
...
class MoodSocialForecastHorizon(MoodSocialSchemaModel):
    task_id: Literal["forecast_1m", "forecast_2m"]
    months_ahead: Annotated[int, Field(strict=True, ge=1, le=2)]
    risk_probability: Probability
    risk_score: Annotated[int, Field(strict=True, ge=0, le=100)]
    risk_level: Annotated[int, Field(strict=True, ge=0, le=3)]
    high_risk_threshold: Probability
    predicted_high_risk: StrictBool

    @model_validator(mode="after")
    def validate_horizon(self) -> "MoodSocialForecastHorizon":
        expected_months = 1 if self.task_id == "forecast_1m" else 2
        if self.months_ahead != expected_months:
            raise _contract_error(
                "field_validation_error",
                "months_ahead must match task_id",
                field="months_ahead",
            )
        expected_score = int(
            (Decimal(str(self.risk_probability)) * Decimal("100")).quantize(
        ...
        if self.risk_score != expected_score:
            raise _contract_error(
                "field_validation_error",
                "risk_score must use half-up rounding",
                field="risk_score",
            )
        if self.predicted_high_risk != (
            self.risk_probability >= self.high_risk_threshold
        ):
            raise _contract_error(
                "field_validation_error",
                "predicted_high_risk must match the frozen threshold",
                field="predicted_high_risk",
            )
        return self


class MoodSocialForecastResult(MoodSocialSchemaModel):
    model_version: Literal["mood-forecast-v3.4.0"]
    artifact_run_id: Literal["MH-20260810-FDEP-001"]
    available: StrictBool
    mode: Literal["state_aware", "passive_fallback", "insufficient_data"]
    historical_phq9_used: StrictBool
    historical_phq9_count: StrictNonNegativeInt
    used_sources: list[Literal["camera", "sleep_device", "historical_phq9"]]
    horizons: Annotated[list[MoodSocialForecastHorizon], Field(max_length=2)]
    evidence_scope: Literal[
        "formal_oof_state_aware", "formal_oof_pure_passive", "unavailable"
    ]
    evaluated_auprc_1m: Probability | None
    evaluated_auprc_2m: Probability | None
    summary: NonEmptyText
    limitations: list[NonEmptyText]
    diagnosis: DiagnosisFalse

    @model_validator(mode="after")
    def validate_forecast(self) -> "MoodSocialForecastResult":
        if self.diagnosis:
            raise _contract_error(
                "field_validation_error",
        ...
        if not self.available:
            if (
                ...
                or self.horizons
                or self.evidence_scope != "unavailable"
                or self.evaluated_auprc_1m is not None
                or self.evaluated_auprc_2m is not None
            ):
                raise _contract_error(
                    "field_validation_error",
                    "unavailable forecast shape is inconsistent",
                    field="available",
                )
            return self
        if [item.task_id for item in self.horizons] != [
            "forecast_1m",
            "forecast_2m",
        ]:
            raise _contract_error(
                "field_validation_error",
                "available forecast must contain ordered 1m and 2m horizons",
                field="horizons",
            )
        if self.mode == "state_aware":
            if (
                not self.historical_phq9_used
                or self.historical_phq9_count < 1
                or "historical_phq9" not in self.used_sources
                or self.evidence_scope != "formal_oof_state_aware"
            ):
                raise _contract_error(
                    "field_validation_error",
                    "state-aware forecast requires strict historical PHQ-9 evidence",
                    field="mode",
                )
        elif (
            self.historical_phq9_used
            or self.historical_phq9_count != 0
            or "historical_phq9" in self.used_sources
            or self.evidence_scope != "formal_oof_pure_passive"
        ):
            raise _contract_error(
                "field_validation_error",
                "passive fallback must not claim historical PHQ-9 evidence",
                field="mode",
            )
        return self
...
class MoodSocialInferResponse(MoodSocialSchemaModel):
    ...
    model_version: Literal[
        "mood-fusion-v3.3.3", "mood-social-current-state-production-r5"
    ]
    available: StrictBool
    ...
    limitations: list[NonEmptyText]
    diagnosis: DiagnosisFalse
    current_state_model_version: NonEmptyText = MOOD_SOCIAL_MODEL_VERSION
    current_state_package_run_id: NonEmptyText = "MH-20260802-013"
    current_state_fallback_used: StrictBool = True
    current_state_fallback_reason: NonEmptyText | None = None
    current_state_evidence_signature: Literal[
        "profile", "sleep", "joint", "unsupported", "legacy"
    ] = "legacy"
    current_state_projection: Literal["none", "profile_only", "sleep_only"] = "none"
    probability_phq_ge5: Probability | None = None
    probability_phq_ge10: Probability | None = None
    forecast: MoodSocialForecastResult | None = None
    ...
    def validate_response_relationships(self) -> "MoodSocialInferResponse":
        ...
        if self.probability_phq_ge5 is not None and self.probability_phq_ge10 is not None:
            if self.probability_phq_ge10 > self.probability_phq_ge5:
                raise _contract_error(
                    "field_validation_error",
                    "PHQ-9 >=10 probability must not exceed PHQ-9 >=5 probability",
                    field="probability_phq_ge10",
                )
        if self.current_state_package_run_id == "MH-20260812-R5-001":
            if (
                self.current_state_model_version
                != "mood-social-current-state-v3.3.3-r5"
                or self.current_state_fallback_used
                or self.current_state_evidence_signature not in {"profile", "sleep"}
                or self.current_state_projection
                not in {"none", "profile_only", "sleep_only"}
                or self.probability_phq_ge5 is None
                or self.probability_phq_ge10 is None
                or self.attention_index != self.probability_phq_ge10
            ):
                raise _contract_error(
                    "field_validation_error",
                    "r5 production responses require ordered dual-head "
                    "probabilities and no fallback",
                    field="current_state_package_run_id",
                )
        if not self.available and (
            self.probability_phq_ge5 is not None
            or self.probability_phq_ge10 is not None
        ):
            raise _contract_error(
                "field_validation_error",
                "unavailable current-state responses must not expose probabilities",
                field="available",
            )
        ...
        for contribution in self.model_contributions:
            allowed_state_branches = state_factor_domains.get(contribution.factor)
            if allowed_state_branches is not None and covered.isdisjoint(
                allowed_state_branches
            ):
                raise _contract_error(
                    "field_validation_error",
            ...
            explanation_text = f"{contribution.factor} {contribution.label}".lower()
            uses_change_language = any(
                marker in explanation_text for marker in change_markers
            )
            ...
            if any(
                marker in explanation_text for marker in prohibited_explanation_markers
            ):
                raise _contract_error(
                    "field_validation_error",
