...
class DatasetSpec:
    ...
    def canonical_relative_path(self) -> Path:
        return PROCESSED_RELATIVE / self.directory_name / self.canonical_name

    @property
    def artifact_manifest_relative_path(self) -> Path:
        return PROCESSED_RELATIVE / self.directory_name / "artifact_manifest.json"

    @property
    def adapter_metadata_relative_path(self) -> Path:
        return PROCESSED_RELATIVE / self.directory_name / "adapter_metadata.json"

    @property
    def mapping_relative_path(self) -> Path:
        return PROCESSED_RELATIVE / "mappings" / self.mapping_name
...
def _validate_schema_binding(repository_root: Path) -> dict[str, str]:
...
def _build_fold_summaries(
    ...
    for outer_fold in range(OUTER_FOLD_COUNT):
        ...
        outer_summaries.append(
            {
                "outer_fold": outer_fold,
                "test": _count_partition(outer_test),
                "train": _count_partition(outer_train),
            }
        )
            ...
            inner_folds.append(
                {
                    ...
                    "train": _count_partition(inner_train),
                    "validation": _count_partition(validation),
                }
            )
        inner_summaries.append(
            {
                "folds": inner_folds,
                "outer_fold": outer_fold,
                "outer_train": _count_partition(outer_train),
            }
        )
...
def build_split_manifest(
    *,
    ...
    schema_binding = _validate_schema_binding(root)
