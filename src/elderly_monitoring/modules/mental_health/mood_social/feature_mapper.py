...
def _aggregate_activity(
    ...
    *,
    ...
    values["activity_volume_norm"] = _activity_volume_ratio(observed_activities)
...
def _activity_volume_ratio(
    activities: Iterable[MoodSocialActivity],
) -> float | None:
    """Return observation-minute-weighted activity intensity.

    Runtime records may provide ``weighted_daytime_activity`` directly or only
    hourly intensities.  Both representations have the same unit
    (intensity-minutes), so the aggregate is the sum divided by observed
    camera minutes.  This is the runtime synonym of the frozen canonical
    ``activity.activity_volume_norm`` feature.
    """
    ...
    for item in activities:
        if item.valid_daytime_detection_minutes <= 0.0:
            continue
        weighted = item.weighted_daytime_activity
        if weighted is None:
            weighted = _hourly_weighted_activity(item)
        if weighted is None:
            continue
        numerator += float(weighted)
        denominator += float(item.valid_daytime_detection_minutes)
    if denominator <= 0.0:
        return None
    return _clip01(numerator / denominator)
