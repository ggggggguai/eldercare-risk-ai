...
from elderly_monitoring.modules.mental_health.mood_social.forecast_runtime import (
...
from elderly_monitoring.modules.mental_health.mood_social.production_current_state import (
...
def infer_mood_social(
    ...
    *,
    ...
    primary_predictor: Callable[
        [MoodSocialInferRequest], Mapping[str, Any]
    ] = predict_primary_current_state,
) -> MoodSocialInferResponse:
    """Run r5 current-state inference with the frozen V3.3.3 safety fallback."""
    current = config or load_mood_social_config()
    package_directory = current.models.directory / "packages" / "MH-20260802-013"
    legacy_response: MoodSocialInferResponse | None = None
    ...
    try:
        ...
        legacy_response = runtime.predict(request)
    except Exception as exc:
        legacy_error = exc

    try:
        current_state = apply_primary_current_state(
            request,
            config=current,
        ...
        try:
            forecast = predict_mood_forecast(request)
        except Exception as exc:  # keep the V3.3.3 current-state branch available
            forecast = unavailable_forecast(
                f"forecast runtime failed safely: {type(exc).__name__}"
            )
        return current_state.model_copy(
            update={"forecast": MoodSocialForecastResult.model_validate(forecast)}
        )
    except Exception as exc:
...
def _classify_validation_error(
    ...
    if _has_early_field_error(errors):
        return "FIELD_VALIDATION_ERROR"
    if "date_validation_error" in error_types or _raw_date_relationship_invalid(body):
        return "DATE_VALIDATION_ERROR"
    if "source_declaration_mismatch" in error_types or _raw_source_declaration_mismatch(
        body
    ):
...
def _is_date_collection_length_error(
    ...
    *,
    ...
    return bool(body_location and body_location[0] in date_collection_fields)
...
def _has_early_field_error(
    ...
    for error in errors:
        ...
        if isinstance(context, Mapping) and context.get("field") == "available_sources":
            return True
    return False
...
def _raw_date_relationship_invalid(body: object) -> bool:
    ...
    if isinstance(history, list):
        ...
        if len(history_dates) == len(history) and all(
            ...
            if concrete_history_dates != sorted(concrete_history_dates) or len(
                concrete_history_dates
            ) != len(set(concrete_history_dates)):
                return True
            earliest = target_date - timedelta(days=28)
            latest = target_date - timedelta(days=1)
            if any(not earliest <= value <= latest for value in concrete_history_dates):
                return True
    ...
    if isinstance(attention_history, list):
        ...
        if len(attention_dates) == len(attention_history) and all(
            ...
            if (
                ...
                or len(concrete_attention_dates) != len(set(concrete_attention_dates))
...
def _raw_source_declaration_mismatch(body: object) -> bool:
    ...
    for record in raw_records:
        ...
        if record.get("activity") is not None and "camera" not in declared:
            return True
        if (
            record.get("sleep") is not None or record.get("physiology") is not None
