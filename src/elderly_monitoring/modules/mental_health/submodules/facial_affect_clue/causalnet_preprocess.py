...
def pair_input(
    ...
    target: np.ndarray,
    config: CausalNetPreprocessConfig,
...
def build_four_route_input_from_aligned_frames(
    onset_frame: np.ndarray,
    apex_frame: np.ndarray,
    offset_frame: np.ndarray,
    config: CausalNetPreprocessConfig | None = None,
) -> tuple[np.ndarray, dict[str, Any]]:
    """Build the frozen four routes from already aligned 256x256 BGR frames.

    This is the online array entry point.  The offline entry point below performs
    its historical affine alignment first and then delegates here, so both paths
    execute the same flow, normalization, direction-map and ROI implementation.
    """
    config = config or CausalNetPreprocessConfig()
    frames = (onset_frame, apex_frame, offset_frame)
    for frame in frames:
        if not isinstance(frame, np.ndarray) or frame.shape != (256, 256, 3):
            raise ValueError(
                "Aligned CausalNet frames must have shape (256, 256, 3)"
            )
        if not np.isfinite(frame).all():
            raise ValueError("Aligned CausalNet frame contains NaN or Inf")
    oa_flow, oa_direction, oa_scale = pair_input(frames[0], frames[1], config)
    ao_flow, ao_direction, ao_scale = pair_input(frames[1], frames[2], config)
    inputs = np.stack((oa_flow, ao_flow, oa_direction, ao_direction), axis=0)
    inputs = np.ascontiguousarray(
        np.transpose(inputs, (0, 3, 1, 2)), dtype=np.float32
    )
    ...
    if not np.isfinite(inputs).all():
        raise ValueError("CausalNet input contains NaN or Inf")
    return inputs, {
        "route_order": [
            "onset_to_apex_flow",
            "apex_to_offset_flow",
...
def build_four_route_input(
    ...
    inputs, array_metadata = build_four_route_input_from_aligned_frames(
        aligned[onset], aligned[apex], aligned[offset], config
    )
    metadata = {
        "task_id": "FLOW-ME-004",
        "schema_version": CAUSALNET_PREPROCESS_SCHEMA_VERSION,
        ...
        "route_order": array_metadata["route_order"],
        ...
        "flow_normalization_parameters": array_metadata[
            "flow_normalization_parameters"
        ],
        "input_shape": array_metadata["input_shape"],
        "preprocess_config_sha256": array_metadata[
            "preprocess_config_sha256"
        ],
        "preprocessing_config_hash": array_metadata[
