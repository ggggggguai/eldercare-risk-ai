...
from .causalnet_dataset import (
    CausalNetArtifactDataset,
    CausalNetAugmentationConfig,
    collate_causalnet,
)
...
CAUSALNET_MULTI_DATASET_EVALUATION_SCHEMA_VERSION = (
    "causalnet_multi_dataset_subject_isolated_v1"
)
...
class CausalNetTrainingConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.sampler not in {"random", "class_balanced"}:
            raise ValueError(f"Unsupported sampler: {self.sampler}")
        if not 0.0 <= self.label_smoothing <= 0.2:
            raise ValueError("label_smoothing must be in [0, 0.2]")
        if self.focal_gamma <= 0.0:
            raise ValueError("focal_gamma must be positive")
        if self.domain_alignment not in {"none", "coral", "mmd"}:
            raise ValueError(f"Unsupported domain alignment: {self.domain_alignment}")
        if self.domain_alignment_weight < 0.0:
            raise ValueError("domain_alignment_weight must be non-negative")
    ...
    *,
    ...
    if mode == "weighted_cross_entropy":
        return nn.CrossEntropyLoss(
            weight=class_weights(records).to(device),
...
def _loader(
    ...
    *,
    ...
    sampler = None
    if shuffle and training_config.sampler == "class_balanced":
        source = list(records or dataset.records)
        weights = class_weights(source)
        sample_weights = torch.tensor(
            [float(weights[int(record["label"])]) for record in source],
            dtype=torch.double,
        )
...
def _coral_loss(features: torch.Tensor, domains: Sequence[str]) -> torch.Tensor:
    unique = sorted(set(domains))
    if len(unique) < 2:
        return features.new_zeros(())
    groups = [features[[index for index, domain in enumerate(domains) if domain == name]] for name in unique]
    losses = []
    for left_index, left in enumerate(groups):
        for right in groups[left_index + 1 :]:
            ...
            left_covariance = left_centered.T @ left_centered / (left.shape[0] - 1)
            right_covariance = right_centered.T @ right_centered / (right.shape[0] - 1)
            mean_loss = (left.mean(dim=0) - right.mean(dim=0)).pow(2).mean()
            covariance_loss = (left_covariance - right_covariance).pow(2).mean()
            losses.append(mean_loss + covariance_loss)
    return torch.stack(losses).mean() if losses else features.new_zeros(())


def _mmd_loss(features: torch.Tensor, domains: Sequence[str]) -> torch.Tensor:
    unique = sorted(set(domains))
    if len(unique) < 2:
        return features.new_zeros(())
    means = [features[[index for index, domain in enumerate(domains) if domain == name]].mean(dim=0) for name in unique]
    return torch.stack(
        [(left - right).pow(2).mean() for index, left in enumerate(means) for right in means[index + 1 :]]
    ).mean()
...
def train_epoch(
    ...
    *,
    ...
    for batch in loader:
        ...
        classification_loss = loss_function(model.mlp_head(features), labels)
        if domain_alignment == "coral":
            alignment_loss = _coral_loss(features, batch["evaluation_datasets"])
        elif domain_alignment == "mmd":
            alignment_loss = _mmd_loss(features, batch["evaluation_datasets"])
        else:
            alignment_loss = features.new_zeros(())
        loss = classification_loss + domain_alignment_weight * alignment_loss
        ...
        total_classification_loss += float(classification_loss.detach().cpu()) * count
        total_alignment_loss += float(alignment_loss.detach().cpu()) * count
        sample_count += count
    denominator = max(sample_count, 1)
    return {
        "loss": total_loss / denominator,
        "classification_loss": total_classification_loss / denominator,
        "alignment_loss": total_alignment_loss / denominator,
    }
            ...
            rows.append(
                {
                    ...
                    "evaluation_dataset": batch["evaluation_datasets"][index],
...
def run_nested_loso_fold(
    *,
    ...
    task_id: str = "EVAL-ME-004",
    evaluation_schema_version: str = CAUSALNET_EVALUATION_SCHEMA_VERSION,
) -> dict[str, Any]:
    ...
    augmentation = CausalNetAugmentationConfig(**dict(training_config.augmentation or {}))
    ...
    for epoch in range(1, training_config.max_epochs + 1):
        training = train_epoch(
            ...
            domain_alignment=training_config.domain_alignment,
            domain_alignment_weight=training_config.domain_alignment_weight,
        )
        ...
        epoch_rows.append(
            {
                ...
                "training_classification_loss": training["classification_loss"],
                "training_alignment_loss": training["alignment_loss"],
