...
| 复现 | [徘徊步骤 10 runtime 前置治理](../reports/mental_health/wandering_step10_runtime/README.md) | Linux/x86_64 `2.13.0+cu130` fact source 已由候选检查点的全新 checkout/环境按 27 个 Conda 工件和 30 个 pip wheel 精确复验；12-path source G0 也已由检查点 `10d36ee` 和 fresh worktree 关闭，仍未创建步骤 10 红测、源码、production config 或训练制品 |
