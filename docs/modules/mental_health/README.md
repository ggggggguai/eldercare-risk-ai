...
证据边界没有扩大到现实效果：RF 与 TCN 仍是 `comparison_only` 且概率未经校准；步骤 7 只证明 `synthetic_camera_contract`，步骤 8 只增加 `synthetic_camera_corruption` 下的确定性增强、Camera QC 压力测试和冻结模型兼容性报告。步骤 9/9a 已形成独立 Git 检查点并恢复 `step9_forward_contract_verified` 与 `step9a_exact_config_fail_closed_verified`；它没有读取数据或训练，不能视为识别效果。没有授权 MP4/目标摄像头真值，不报告摄像头准确率、召回率或误报率。该子包仍未从本模块顶层重新导出，也未接入现有聚合、评分或运行时；TopoWander-MPT 训练、校准/OOD、episode、日级徘徊字段、共享心理风险主链和正式现实效果指标仍未实现。
...
方案步骤 9/9a 的 `configs/modules/wandering_topowander_mpt_v1.yaml` SHA-256 仍为 `debd3adcf0c84d266ca594fd9ba2716a5a854e074e6e2092570b1ec656ed80f7`，三输入、mask-aware TCN、19 个语义 patch、对称 relation + 37 档 signed offset、两层显式 relation-bias Transformer、三个 head、204,466 参数、77 state key 和 deterministic NPZ 均未漂移。文件安全 loader 只读一次 config bytes 并以同一 payload 完成 SHA/解析；普通 dict/list 构造后无别名、公开嵌套只读、属性重绑定拒绝和输出顺序测试也成立。第 9a.6 节新增顶层/嵌套状态化 Mapping 红测，旧 `a5ea…c554ab` 候选因成功构造产生 `3 failed`；修正后构造器/工厂只接受 plain `dict[str, Any]`，单次递归捕获 built-in dict/list 与冻结标量为私有 snapshot，再对同一 snapshot 校验并冻结运行时视图，自定义/惰性 Mapping 在 `_build_modules()` 前失败。
...
下一项不是继续修改步骤 9 或 runtime：步骤 9+9a 检查点、最终 Step 9 绑定和 Linux/x86_64 `2.13.0+cu130` runtime fact source 均已形成；fact source=`configs/runtime/wandering_step10_runtime_v1.yml / 11426 bytes / 034d603a…e2f1cda`，候选 commit `03079a5` 的全新 checkout/环境重建、27/30 工件来源审计、`pip check` 和版本/平台/thread/deterministic 探针通过。步骤 10 已冻结固定 v3 pair-aware 路线：1,257 个 clean train parent、1,491 条 pair、126 个 train-internal proxy、输入级 20-cell mask、只调用 `encode()` 的 wrapper exact graph、CLI、早停、20-file DAG、config/artifact schema 和 canonical bytes。终审新增的 G0 尚未关闭：22-source closure 中 12 个辅助源码当前是 `i/lf,w/crlf,attr/`，须先形成 exact LF 属性、Git 检查点与 fresh-checkout SHA；通过后才按“红测→源码/CLI→源码 SHA→production YAML”推进。production config 外部 SHA 前仍不得读取正式训练数据、创建正式数据路径的 production optimizer 或生成制品，合成测试可按契约创建 AdamW。完整证据见[步骤 10 runtime 前置治理](../../../reports/mental_health/wandering_step10_runtime/README.md)。

G0 当前唯一允许的工程变更是给 `.gitattributes` 增加技术方案明列的 12 条 exact path rule并形成新检查点，不能先创建红测；G0 通过后，首个步骤 10 新文件才是 `tests/test_wandering_pretraining.py`。开发 pytest 使用标准 `eldercare-ai` 和合成 fixture；两份生产源码转绿后还须形成 LF/source checkpoint 与 fresh-checkout 证据，production config 外部 SHA 后再经负责人授权执行六文件 R2M。正式流程只使用 frozen runtime YAML 创建的两个全新环境 A/B，并以 `python -m pip install --no-deps --no-build-isolation -e .` 接入 active checkout。每个环境启动一个训练 CLI fresh process，由唯一 builder 在同一进程内部先 preflight 后构建自身 bundle，再用一个 fresh process 完成 full verifier、用五个 fresh process 分别执行显式 seed safe loader；不存在独立 preflight API。训练 builder 的 32-file 显式输入/源码闭包、verifier 的 45-file 集合、denylist 和唯一阶段顺序见技术方案第 10.0–10.2、10.8–10.9 节。
...
Windows 当前需从 WSL `Ubuntu-22.04` 调用 `/home/lenovo/miniconda3/bin/conda run -n eldercare-ai`。步骤 9 没有训练、评估或导出 CLI，也没有正式 checkpoint。
