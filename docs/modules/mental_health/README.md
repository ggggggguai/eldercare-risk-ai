...
下一项不是继续修改步骤 9、runtime 或 source G0：步骤 9+9a 检查点、最终 Step 9 绑定和 Linux/x86_64 `2.13.0+cu130` runtime fact source 均已形成；fact source=`configs/runtime/wandering_step10_runtime_v1.yml / 11426 bytes / 034d603a…e2f1cda`，候选 commit `03079a5` 的全新 checkout/环境重建、27/30 工件来源审计、`pip check` 和版本/平台/thread/deterministic 探针通过。22-source closure 的 12-path G0 也已由检查点 `10d36ee` 和 fresh worktree 关闭，12 项 raw bytes/SHA 与冻结目标一致且源码 blob 零变更。步骤 10 已冻结固定 v3 pair-aware 路线：1,257 个 clean train parent、1,491 条 pair、126 个 train-internal proxy、输入级 20-cell mask、只调用 `encode()` 的 wrapper exact graph、CLI、早停、20-file DAG、config/artifact schema 和 canonical bytes。现在按“R0 红测→源码/CLI→源码 SHA→production YAML”推进；production config 外部 SHA 前仍不得读取正式训练数据、创建正式数据路径的 production optimizer 或生成制品，合成测试可按契约创建 AdamW。完整证据见[步骤 10 runtime 前置治理](../../../reports/mental_health/wandering_step10_runtime/README.md)。

G0 已由 `10d36ee` 关闭；首个步骤 10 新文件现在只能是 `tests/test_wandering_pretraining.py`。开发 pytest 使用标准 `eldercare-ai` 和合成 fixture；两份生产源码转绿后还须形成 LF/source checkpoint 与 fresh-checkout 证据，production config 外部 SHA 后再经负责人授权执行六文件 R2M。正式流程只使用 frozen runtime YAML 创建的两个全新环境 A/B，并以 `python -m pip install --no-deps --no-build-isolation -e .` 接入 active checkout。每个环境启动一个训练 CLI fresh process，由唯一 builder 在同一进程内部先 preflight 后构建自身 bundle，再用一个 fresh process 完成 full verifier、用五个 fresh process 分别执行显式 seed safe loader；不存在独立 preflight API。训练 builder 的 32-file 显式输入/源码闭包、verifier 的 45-file 集合、denylist 和唯一阶段顺序见技术方案第 10.0–10.2、10.8–10.9 节。
