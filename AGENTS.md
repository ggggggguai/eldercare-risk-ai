...
当前项目处于跌倒风险正式模型替换完成并冻结阶段。2026-08-19 项目负责人接受现有证据边界，将当前混合运行基线冻结为比赛正式交付版本：步态 seed 43、坐站 seed 42 和跌倒事件三 seed TCN 保持默认主评分；近跌倒、个体基线和最终风险融合保持当前规则/统计实现；所有模型不可用、输入不兼容或推理失败场景继续保留规则 fallback。冻结清单以 `configs/modules/fall_risk_release_v1.yaml` 为事实源，后续模型、阈值或服务配置更新必须使用新的 release ID 并重新验证，不能原地覆盖。
