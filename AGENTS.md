...
本项目是面向比赛交付的算法原型，不按生产级平台做全面建设。开发决策首先服务于实际算法效果、创新性、演示完成度和答辩竞争力，避免为了架构完整、形式规范或未来可能的需求过度工程化。

- 资源优先投入数据质量与标注、有效特征与模型、对照和消融实验、真实视频验证，以及准确率、召回率、误报率、延迟和跨场景泛化等可量化结果。
- 优先采用能够快速验证假设的最简单方案；已有代码可以满足实验和演示需求时，不新增通用框架、额外抽象层、复杂配置系统或与当前比赛无关的平台能力。
- 新增基础设施、重构、测试和文档前，先说明它对算法指标、实验可信度、演示稳定性或答辩证据的直接贡献；没有明确贡献时默认不做或缩小范围。
- 测试和兼容性建设按当前风险与比赛使用场景控制范围，不为假设性的生产流量、未知客户端或远期扩展穷举设计。
- 当方案复杂度、算力或时间成本明显高于预期收益时，及时停止并回到可测量的算法改进，不以代码量、模块数量或架构复杂度作为进展。
- 比赛导向不等于牺牲结果真实性。数据防泄漏、指标口径、必要回归测试、实验可复现性和对当前能力边界的准确表述仍是最低要求。
...
当前项目处于跌倒风险比赛交付 v2 冻结阶段。2026-08-27 项目负责人接受现有证据边界，将 `fall-risk-competition-v2-20260827` 冻结为当前正式比赛交付版本：步态 seed 43、连续坐站 seed 42、近跌倒 ExtraTrees seed 42 和跌倒事件三 seed TCN 为默认主评分；近跌倒先由规则生成候选再用因果窗口重评分，个体基线和最终风险融合保持统计/规则实现；所有模型不可用、输入不兼容或推理失败场景继续保留规则 fallback。冻结清单以 `configs/modules/fall_risk_release_v2.yaml` 为事实源，v1 保持不可变用于回滚；后续模型、阈值、运行实现或服务配置更新必须使用新的 release ID 并重新验证，不能原地覆盖。

“正式模型替换完成”是比赛交付与运行版本决策，不追溯改写实验事实。当前 v2 formal blocker、v3 `action_type=false`、未冻结的研究 split/协议、未读取 test、连续背景/老人域和纵向真值缺口继续作为已知限制保留；不得据此宣称临床有效、完整泛化验证通过或虚构缺失指标。历史报告中的 `development_provisional`/`No-Go` 表示当时按预注册研究门禁所得结论，冻结决定只表示负责人接受当前混合实现作为正式比赛基线。
