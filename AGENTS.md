...
心理健康徘徊专项采用 TCN + Transformer 轨迹 shape 主模型；B01+B02 automatic episode+shape development、W5D-03A 三帧多模态 context core、W5D-04 真实 development 日报/个人滚动基线、W5D-05 后端可消费 JSON/JSONL handoff/E2E、W5D-03B 居家 smoke 和 70-task labeled-development 复核均已完成。2026-08-22 的居家问题修复已完成：0.70 可信轨迹门槛、召回优先分段、统一送模门禁、camera geometry subtype 和 parent/resolved 层级输出已落地；修复后 parent boundary precision/recall/F1 为 `0.802920/0.814815/0.808824`，resolved boundary recall 为 `0.938931`，全链 direct/pacing recall 为 `0.933962/0.888889`，random recall 仍为 `0.25`。结果只属于同批 development。task 1657 / track 108 已由负责人裁决为 `direct + ordinary_negative`；CVAT rectangle 与 `evaluation_role` 只作时间段/行为语义，不是检测/跟踪真值。`wand_H02_dining_1_exercise` / track 100 的 evaluation role 仍需人工裁决，算法侧不得擅改。修复记录见 `docs/tasks/徘徊居家视频分段与分类修复.md`，当前任务以 `docs/tasks/README.md` 为唯一状态源，技术语义以 `docs/modules/mental_health/plans/徘徊识别技术文档2.md` 为准。后端和前端不在本轮实现；独立 participant/setup、C4 sealed、PORTABLE 和长期临床验证均不阻塞当前闭环。旧 Step9/10、EP1B/EP2A/MVP/PORTABLE 任务书和报告只用于历史追溯或复用已实现原语，不得用其中已被 superseded 的门禁暂停当前任务。
