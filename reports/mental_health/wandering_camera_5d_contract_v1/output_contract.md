...
- `episode_results`：shape/binary 结果；proposal、QC 和 run status 分开保存。`uncertain` 可送 shape，但不改写为 accepted boundary。
...
conda run -n eldercare-ai python scripts/wandering/build_camera_delivery_contract.py \
  ...
  --output reports/mental_health/wandering_camera_5d_contract_v1
```
